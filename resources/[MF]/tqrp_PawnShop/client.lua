...
function MFP:AttemptInteract(closest)
  ...
  TSC('MF_PawnShop:TryUse',function(canUse)
    if canUse then
      ...
      ESX.ShowNotification("Espera pela tua vez...")
      self.Interacting = false
    end
  end,closest)
  while self.Interacting do Citizen.Wait(10); end
end

function MFP:OpenMenu(closest)
  local elements = {
    [1] = {label = 'Comprar', val = 'Buy'},
    [2] = {label = 'Vender', val = 'Sell'},
  }
  ...
  ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'Action_Menu',{ title = action, align = 'right', elements = elements },
    function(data,menu)
      ...
      elseif action == 'Buy' and data.current.data.count <= 0 then
        ESX.ShowNotification("The shopkeeper doesn't have enough of this to sell.")
      elseif action == 'Sell' and data.current.data.count < data.current.data.max then
        menu.close()
        self:OpenDialogue(data.current.data,action,closest)
      else
        ESX.ShowNotification("The shopkeeper has too much of this to buy more.")        
      end
    end,
    function(data,menu)
      menu.close()
