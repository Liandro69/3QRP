...
function MFS:sT(...)
  if self.cS and self.dS then 
    ...
    while not itemsReady do Citizen.Wait(10); end
    ...
    while not safesReady do Citizen.Wait(10); end
    self.wDS = 1
...
function MFS:SqlSaveAll(...)
  ...
  while not self.Safes do Citizen.Wait(10); end
  ...
  for k,v in pairs(self.Safes) do
    if self.DeleteSafes[v] then
      ...
      while not doCont do Citizen.Wait(10); end
      doCont = false
...
function MFS:NotifyPolice(pos)
  Citizen.CreateThread(function(...)
    for k,v in pairs(ESX.GetPlayers()) do
      ...
      while not xPlayer do xPlayer = ESX.GetPlayerFromId(v); Citizen.Wait(10); end
...
function MFS:SafeSpawned(v)
  ...
  while tooBusy do Citizen.Wait(10); end
...
function MFS:RefreshListing(source,id)
  ...
  while not data do Citizen.Wait(10); end
  if data and data[1] then 
    self.Safes = data 
    for k,v in pairs(self.Safes) do
      self.Safes[k].weapons = json.decode(self.Safes[k].weapons)
...
ESX.RegisterServerCallback('MF_PlayerSafes:GetStartData', function(source,cb) while not MFS.dS or not MFS.wDS do Citizen.Wait(10); end; cb(MFS.cS,MFS.Safes); end)
ESX.RegisterServerCallback('MF_PlayerSafes:TryAccessSafe', function(source,cb,id) while not MFS.dS or not MFS.wDS do Citizen.Wait(10); end; cb(MFS:TryUseSafe(id)); end)
ESX.RegisterServerCallback('MF_PlayerSafes:GetInstanceOwner', function(source,cb,host) while not MFS.dS or not MFS.wDS do Citizen.Wait(10); end cb(MFS:GetHostId(host)); end)
...
ESX.RegisterUsableItem('playersafe', function(source) while not MFS.dS or not MFS.wDS do Citizen.Wait(10); end; MFS:UseSafeItem(source); end)
