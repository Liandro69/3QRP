...
RegisterNetEvent("tqrp_inventoryhud:openPropertyInventory")
AddEventHandler("tqrp_inventoryhud:openPropertyInventory", function(data, playerSafe)
    ...
    function(data, cb) 
        ...
        if type(data.number) == "number" and math.floor(data.number) == data.number then
            ...
            if isPlayerSafe then       
                ...
                TriggerServerEvent("tqrp_property:putItem", ESX.GetPlayerData().identifier, data.item.type, data.item.name, count)
            end
        end
    ...
    function(data, cb)
        ...
        if type(data.number) == "number" and math.floor(data.number) == data.number then
            if isPlayerSafe then
                ...
                TriggerServerEvent("tqrp_property:getItem", ESX.GetPlayerData().identifier, data.item.type, data.item.name, tonumber(data.number))
            end
        end
