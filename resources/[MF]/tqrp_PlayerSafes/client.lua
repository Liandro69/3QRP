        ...
        ESX.TriggerServerCallback('MF_PlayerSafes:TryAccessSafe',function(inUse)
          if inUse then
            ...
            if not isOwner then
              ...
              while self.CrackingSafe do Citizen.Wait(10); end
            else
              self.NUIClosed = false
              self.UsingSafe = closestSafe
              ESX.TriggerServerCallback('MF_PlayerSafes:GetSafeInventory', function(inventory)
                TriggerEvent("tqrp_inventoryhud:openPropertyInventory", inventory, closestSafe)
...
function MFS:EndMinigame(didWin)
  ...
  if didWin then
    ESX.TriggerServerCallback('MF_PlayerSafes:GetSafeInventory', function(inventory)
      TriggerEvent("tqrp_inventoryhud:openPropertyInventory", inventory, safe)
    end, safe.safeid)     
    self.NUIClosed = false
    self.UsingSafe = safe
...
function MFS:SpawnThisSafe(key,pos)
  ...
  while not self.SpawnedSafes[key] do Citizen.Wait(10); end
end
...
function MFS:EnterInstance(instance)  
  ...
  while not self.Instance do Citizen.Wait(10); end
end
...
AddEventHandler('tqrp_inventoryhud:closeInventory', function(...) MFS.NUIClosed = true; end)

RegisterNetEvent('instance:onEnter')
AddEventHandler('instance:onEnter', function(instance) while not MFS.Safes do Citizen.Wait(10); end; MFS:EnterInstance(instance); end)

RegisterNetEvent('instance:onLeave')
AddEventHandler('instance:onLeave', function(instance) while not MFS.Safes do Citizen.Wait(10); end; MFS:LeaveInstance(); end)
