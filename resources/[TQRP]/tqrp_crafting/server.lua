...
function crafting:Awake(...) 
  while not self.SqlReady do Citizen.Wait(10); end
  self.CraftingTables = {}
...
function crafting:CraftItem(source,name,label,learn)
  ...
  if recipe then
    ...
    if hasAll then
      ...
      if  name == 'WEAPON_DBSHOTGUN' or name == 'WEAPON_HEAVYPISTOL' or name == 'WEAPON_MACHINEPISTOL' or name == 'WEAPON_PISTOL' or name =='WEAPON_MINISMG' then
        if math.random(1,100) < 21 then
          xPlayer.removeInventoryItem("recipe_"..name, 1)
        end
      end
      if learn then
