...
function MFD:Start()
  ...
  --print("tqrp_provas:Start() -- Successful")
...
function MFD:PositionUpdate()
  ...
  while self.dS and self.cS do
    Citizen.Wait(sleep)
    tick = tick + 1
    if tick % 200 == 0 then ESX.TriggerServerCallback('tqrp_provas:GetJob', function(job) self.CurJob = job.label; end); end
    if self.CurJob == self.AmbulanceJob or self.CurJob == self.PoliceJob then
      ...
      if distA and distA < self.DrawTextDist then
        drawing = "ammo"
        self:DrawText3D(self.AmmoAnalyzePos.x,self.AmmoAnalyzePos.y,self.AmmoAnalyzePos.z, " [~g~E~s~] pressiona para analisar.")
      elseif distB and distB < self.DrawTextDist then
        drawing = "blood"
        self:DrawText3D(self.DNAAnalyzePos.x,self.DNAAnalyzePos.y,self.DNAAnalyzePos.z, " [~g~E~s~] analisa o ADN que está no chão.")
      end
      if drawing then
        sleep = 10
        if (IsControlJustPressed(0, Keys["E"]) or IsDisabledControlJustPressed(0, Keys["E"])) then
          if drawing == "ammo" then TriggerEvent('tqrp_provas:AnalyzeAmmo')
          elseif drawing == "blood" then TriggerEvent('tqrp_provas:AnalyzeDNA')
          end
        end
      else
        sleep = 1500
...
function MFD:HandleDrawText(plyPed)
  ...
  if closestDist and closestDist < 2.0 then
    local str = ''
    if closest.obj == self.BloodObject then str = "[ ~r~DNA~s~ ] : Sangue?? :  [~r~ E ~s~] recolhe amostra." else str = "[ ~r~PROVA ~s~] : Status do projetil :  [~r~ E ~s~] apanha o involucro"; end
    self:DrawText3D(closest.pos.x,closest.pos.y,closest.pos.z, str)
    if (IsControlJustPressed(1, Keys["E"]) or IsDisabledControlJustPressed(0, Keys["E"])) and (GetGameTimer() - self.timer) > 150 then
      self.timer = GetGameTimer()
      ESX.TriggerServerCallback('tqrp_provas:PickupEvidenceS', function(canPickup)
        if not canPickup then exports['mythic_notify']:SendAlert('error', 'Não podes apanhar mais provas')
        else
...
RegisterNetEvent('tqrp_provas:PickupEvidenceC')
AddEventHandler('tqrp_provas:PickupEvidenceC', function(evidence) MFD:PickupEvidence(evidence); end)
...
function MFD:TakeDamage(ped)
  ...
  TriggerServerEvent('tqrp_provas:PlaceEvidenceS', newPos, self.BloodObject)
...
function MFD:PlayerShooting(ped)
  ...
  TriggerServerEvent('tqrp_provas:PlaceEvidenceS', newPos, self.ResidueObject, weapon, weaponType)
...
function MFD:Awake(...)
    while not ESX do Citizen.Wait(100); end
    while not ESX.IsPlayerLoaded() do Citizen.Wait(0); end
    ESX.TriggerServerCallback('tqrp_provas:GetStartData', function(retVal) self.dS = true; self.cS = retVal; self:Start(); end)
end

RegisterNetEvent('tqrp_provas:AnalyzeDNA')
AddEventHandler('tqrp_provas:AnalyzeDNA', function()
  local self = MFD
  ...
  local elements = {
    [1] = { label = "Nome da Pessoa: " .. self.CurBlood.name},
    [2] = { label = "Tipo:" },
    [3] = { label = "Morada: "..GetStreetNameFromHashKey(street) },
    [4] = { label = "Localização: [ X : " .. math.floor(self.CurBlood.pos.x) .. " ] [ Y : " .. math.floor(self.CurBlood.pos.y) .. "] : [ Z : " .. math.floor(self.CurBlood.pos.z).." ]"},
  }
  ESX.UI.Menu.Open('default', GetCurrentResourceName(), "Blood_Info", { title = "Analisador de DNA", align = 'left', elements = elements },
    function(data,menu)
      menu.close()
...
RegisterNetEvent('tqrp_provas:AnalyzeAmmo')
AddEventHandler('tqrp_provas:AnalyzeAmmo', function()
  local self = MFD
  ...
  local elements = {
    ...
    [3] = { label = "Morada: "..GetStreetNameFromHashKey(street) },
    [4] = { label = "Localização: [ X : " .. math.floor(self.CurBullet.pos.x) .. " ] : [ Y : " .. math.floor(self.CurBullet.pos.y) .. "] : [ Z : " .. math.floor(self.CurBullet.pos.z).." ]"},
  }
  ESX.UI.Menu.Open('default', GetCurrentResourceName(), "AnalyzeAmmo", { title = "Analisador de Munição", align = 'left', elements = elements },
    function(data,menu)
      menu.close()
      ESX.UI.Menu.CloseAll()
    end,
...
RegisterNetEvent('tqrp_provas:PlaceEvidenceC')
AddEventHandler('tqrp_provas:PlaceEvidenceC', function(pos, obj, name, weapon, weaponType)
