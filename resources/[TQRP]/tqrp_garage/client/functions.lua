...
SpawnVehicle = function(data,recuperar)
	...
	if not ESX.Game.IsSpawnPointClear(spawnpoint["position"], 3.0) then 
		exports['mythic_notify']:SendAlert('inform', 'Está um veículo muito próximo')
	...
	for i = 1, #gameVehicles do
        ...
        if DoesEntityExist(vehicle) then
			if Config.Trim(GetVehicleNumberPlateText(vehicle)) == Config.Trim(vehicleProps["plate"]) then
                exports['mythic_notify']:SendAlert('error', 'O teu carro já está na cidade!')
	...
	ESX.Game.SpawnVehicle(vehicleProps["model"], spawnpoint["position"], spawnpoint["heading"], function(yourVehicle)
        ...
        TriggerEvent('tqrp_phone:client:updateVehState', vehicleProps.plate, 'OUT', 0)
        HandleCamera(cachedData["currentGarage"])
    end)
    if recuperar then
        exports['mythic_notify']:SendAlert('success', 'Pagaste 200$ para recuperar a tua viatura!')
...
SaveInGarage = function(garage)
    ...
    exports['mythic_notify']:SendAlert('success', 'Veículo guardado '..garage)
    Citizen.Wait(500)

    deleteCar(vehicle)
    TriggerServerEvent('betrayed_garage:modifystate', vehicleProps, 0, garage)
    TriggerEvent('tqrp_phone:client:updateVehState', vehicleProps.plate, garage, 0)
    CloseMenu()
end
...
WaitForModel = function(model)
    ...
    if not IsModelValid(model) then
        return exports['mythic_notify']:SendAlert('inform', 'Modelo inválido, cria um ticket!')
    end
	...
	while not HasModelLoaded(model) do
		...
		DrawScreenText("A procurar por ti... " .. GetDisplayNameFromVehicleModel(model) .. "...", 255, 255, 255, 150)
	end
end