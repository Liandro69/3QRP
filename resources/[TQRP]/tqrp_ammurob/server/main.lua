...
local robbableItemsRare= {
    [1] = {chance = 6, id = 'arma_cano', name = 'Kit Médico', quantity = math.random(2, 4)},
    [2] = {chance = 6, id = 'tec9_body', name = 'Kit Médico', quantity = math.random(3, 5)},
    [3] = {chance = 6, id = 'armacaodb', name = 'Kit Médico', quantity = 1},
    [4] = {chance = 6, id = 'HeavyArmor', name = 'Kit Médico', quantity = 1},
    [5] = {chance = 6, id = 'WEAPON_SNSPISTOL', name = 'Kit Médico', quantity = math.random(1, 2)},
...
local robbableItemsSuperRare= {
    ...
    [6] = {chance = 6, id = 'disc_ammo_shotgun', name = 'Punhal', quantity = 4},
...
local robbableItemsUltraRare= {
    ...
    [3] = {chance = 6, id = 'recipe_WEAPON_HEAVYPISTOL', name = 'Mossberg 500', quantity = 1},
	[4] = {chance = 9, id = 'recipe_WEAPON_MACHINEPISTOL', name = 'Taurus PT92AF', quantity = 1},
	[5] = {chance = 6, id = 'recipe_WEAPON_PISTOL', name = 'Musket M107', quantity = 1},
	[6] = {chance = 4, id = 'net_cracker', name = 'Munição para Pistola', quantity = 1},
	[7] = {chance = 6, id = 'recipe_WEAPON_MINISMG', name = 'Munição para Pistola', quantity = 1},
}
...
RegisterServerEvent('ammurob:start')
AddEventHandler('ammurob:start', function(copsOnDuty, store)

    local _source = source
    if Config.AmmuStores[store].canRob then
         if copsOnDuty >= min_police then
            TriggerClientEvent("ammurob:start", _source, store)
            Config.AmmuStores[store].canRob = false
            --Restart(_source)
         else
            TriggerClientEvent('mythic_notify:client:SendAlert', _source, { type = 'error', text = 'Polícia Insuficiente!' })
        end
    else
        TriggerClientEvent('mythic_notify:client:SendAlert', _source, { type = 'error', text = 'A loja foi assaltada recentemente!' })
    end
