...
local count = 0
local TackleTime = 1500 -- In milliseconds		= 0
local stamina = 0
inVeh = false
local vehicles = {}
...
local Locations = {
	-- CELAS PALETO
	[1] = {From = {334.25, -569.86, 42.6}, To = {319.41, -559.45, 28.1}, DistanceVision = 3, DistanceClick = 1.5, Message = "[~g~E~w~] para descer", Car = false},
	[2] = {From = {319.41, -559.45, 28.1}, To = {334.25, -569.86, 42.6}, DistanceVision = 3, DistanceClick = 1.5, Message = "[~g~E~w~] para subir", Car = false},
	-- Hostpial 2 Heliporto
	[3] = {From = {340.1, -592.59, 42.6}, To = {338.91, -585.38, 74.17}, DistanceVision = 3, DistanceClick = 1.5, Message = "[~g~E~w~] Subir", Car = false},
	[4] = {From = {338.91, -585.38, 74.17}, To = {340.1, -592.59, 42.6}, DistanceVision = 3, DistanceClick = 1.5, Message = "[~g~E~w~] Descer", Car = false},
	-- Vanilla
...
local coordonate = {
	...
	{308.67, -596.87, 42.28, 26.57, 0xD47303AC, "s_m_m_doctor_01"},
    {313.34, -590.9, 42.3, 130.97, 0x22911304, "s_m_y_doorman_01"},
	...
	while true do
		for _,v in pairs(coordonate) do
			if #(vector3(v[1],v[2],v[3]) - GetEntityCoords(PlayerPed)) <= 20 then
				if not DoesEntityExist(ped[_]) then
					ped[_] =  CreatePed(4, v[6],v[1],v[2],v[3], 3374176, false, true)
					SetEntityHeading(ped[_], v[4])
					FreezeEntityPosition(ped[_], true)
					SetBlockingOfNonTemporaryEvents(ped[_], true)
					TaskPlayAnim(ped[_], "mini@strip_club@idles@bouncer@base", "base", 8.0, 0.0, -1, 1, 0, 0, 0, 0)
				end
			else
				if DoesEntityExist(ped[_]) then
					DeletePed(ped[_])
				end
			end
		end
...
AddEventHandler('onyx:beginHotwire', function(plate)
    ...
    exports['mythic_progbar']:Progress({
        ...
        if not status then
            TaskPlayAnim(PlayerPed, "veh@std@ds@base", "hotwire", 8.0, 8.0, -1, 1, 0.6, true, true, true)
            exports['mythic_progbar']:Progress({
                name = "unique_action_name",
                duration = 7000,
                label = "A ligar fios...",
                ...
                if not status then
                    TaskPlayAnim(PlayerPed, "veh@std@ds@base", "hotwire", 8.0, 8.0, -1, 1, 0.4, true, true, true)
                    exports['mythic_progbar']:Progress({
                        name = "unique_action_name",
                        duration = 5000,
                        label = "A ligar o carro...",
                        useWhileDead = false,
                        canCancel = true,
                        controlDisables = {
                            disableMovement = true,
...
Citizen.CreateThread(function() -- CONTROLO NPC
    while true do
		...
		local dist1 = #(coords - vector3(194.23, -925.44, 30.69))
		local dist2 = #(coords - vector3(246.75, -337.17, -118.79))
...
Citizen.CreateThread(function()
	...
	while true do
		...
		--[[SetVehicleDensityMultiplierThisFrame(0.2) -- CONTROLO DE NPCs / PEDS EM CARROS
		SetPedDensityMultiplierThisFrame(0.6)   -- CONTROLO DE NPCs / PEDS
	...
	--[[	if IsControlPressed(0, 168) then
		...
		if not inVeh then
			...
			N_0x4757f00bc6323cfe(GetHashKey("WEAPON_CROWBAR"), 0.35)
		...
		--	N_0x4757f00bc6323cfe(1741783703, 0.0)
		...
		--	SetPedSuffersCriticalHits(PlayerPed, true) 
			...
			if IsPedJumping(PlayerPed) and IsPedRunning(PlayerPed) and stamina > 10 then
		...
		if not inVeh then
			...
			if IsControlPressed(0, 21) and IsControlJustPressed(0, 38) then
				...
				if DoesEntityExist(closestVehicle) then
					local Distance = GetDistanceBetweenCoords(GetEntityCoords(closestVehicle), coords)
...
Citizen.CreateThread(function()
	while true do
		...
		for i=1, #ChopCarLocation, 1 do
			local DistanceCheck = GetDistanceBetweenCoords(coords, ChopCarLocation[i].Chop, true)
		...
		if station ~= nil then
			if GetDistanceBetweenCoords(coords, ChopCarLocation[station].Chop, true) >= 50 and chopstarted then
				tofaraway()
			end
		end
		...
		if coords == prevPos then
			if time > 0 then
				if time == math.ceil(600 / 4) then
					exports["mythic_notify"]:SendAlert("error", "ANTI-AFK - Vais ser Kickado em " .. time .. "segundos por estares AFK")
				end
	...
	while true do
		...
		for i=1, #Locations, 1 do
			if Locations[i].Car then
				if inVeh then
					if GetDistanceBetweenCoords(coords,Locations[i].From[1],Locations[i].From[2],Locations[i].From[3], true) < Locations[i].DistanceVision then
						sleep = 7
						DrawText3Ds(Locations[i].From[1],Locations[i].From[2],Locations[i].From[3]+1.15, Locations[i].Message)
						if GetDistanceBetweenCoords(coords, Locations[i].From[1],Locations[i].From[2],Locations[i].From[3], true) < Locations[i].DistanceClick then
			...
			if GetPedInVehicleSeat(car, -1) == PlayerPed then
				if engineOn then
					if allowedchop and not chopstarted and station ~= nil then
						...
						if GetDistanceBetweenCoords(coords, ChopCarLocation[station].Chop, true ) < 1.2 then
