...
Citizen.CreateThread(function()
	...
	while true do
		Citizen.Wait(sleep)
		sleep = 1500
		if ESX.PlayerData.job and ((ESX.PlayerData.job.name == Config.PoliceDatabaseName[1]) or (ESX.PlayerData.job.name == Config.PoliceDatabaseName[2])) then
...
PoliceGarage = function(type)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), "tqrp_policeGarage_menu",
		{
...
VehicleMenu = function(type)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), "tqrp_policeGarage_vehicle_garage",
		{
			title    = Config.TitlePoliceGarage,
			align    = "center",
			elements = elements
	...
	function(data, menu)
		...
		TriggerEvent("onyx:updatePlates", ESX.Game.GetVehicleProperties(veh).plate)
		if type == 'car' then
		...
		exports["tqrp_base"]:SetFuel(veh, 100)
		...
		for i = 1, #v.Pos, 1 do
			if IsPedInAnyVehicle(ped, true) then
				...
				if GetDistanceBetweenCoords(GetEntityCoords(ped), v.Pos[i].x, v.Pos[i].y, v.Pos[i].z, true) <= Config.Distance then
					--ESX.ShowNotification(Config.VehRetqrp_notification)
					exports['mythic_notify']:SendAlert('inform', 'O teu Veiculo está a ser reparado, espera um pouco!')
					FreezeEntityPosition(veh, true)
					exports['mythic_progbar']:Progress({
						name = "limpar_carro",
						duration = 5000,
						label = "A reparar o veiculo",
						useWhileDead = true,
						canCancel = true,
						controlDisables = {
						...
						if not cancelled then
							...
							exports['mythic_notify']:SendAlert('inform', 'O teu Veiculo foi reparado!')
							SetVehicleFixed(veh)
		...
		for i = 1, #v.Pos, 1 do
			if IsPedInAnyVehicle(ped, true) then
				...
				if GetDistanceBetweenCoords(GetEntityCoords(ped), v.Pos[i].x, v.Pos[i].y, v.Pos[i].z, true) <= Config.Distance then
					...
					exports['mythic_notify']:SendAlert('inform', 'Estamos a limpar o teu veiculo, espera um pouco!')
					--FreezeEntityPosition(veh, true)
					exports['mythic_progbar']:Progress({
						name = "limpar_carro",
						duration = 5000,
						label = "Estão a limpar o carro",
						useWhileDead = true,
						canCancel = true,
						controlDisables = {
							disableMovement = true,
						...
						if not cancelled then
							--ESX.ShowNotification(Config.VehRepDoneNotify)
							exports['mythic_notify']:SendAlert('inform', 'Veiculo Limpo!')
							--SetVehicleFixed(veh)
