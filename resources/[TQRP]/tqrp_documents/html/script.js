...
function Form (_title, _subtitle, _elements, _submittable)
{
	...
	this.submit = function() { 
		...
		if (can_submit) {
			$.post('http://tqrp_documents/form_submit', json_string);
			activeform.close();
		}
	...
	this.print = function () { 
		...
		html_footer += "</div>";
		html_footer += "<div id=\"signature_block\">Clica para assinar.</div>";

		/*  Cancel & Submit button */

		if (this.submittable) {
			html_footer += "<button id=\"button_cancel\" type=\"button\">Cancelar</button>";
			html_footer += "<button id=\"button_submit\" type=\"button\">Submeter</button>";
		}
	...
	this.close = function() {
		...
		$.post('http://tqrp_documents/form_close', JSON.stringify({}));
	}
}
