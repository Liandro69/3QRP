...
function CopyFormToPlayer(aPlayer)
    --TriggerServerEvent('tqrp_documents:CopyToPlayer', GetPlayerServerId(player), aDocument)
    TriggerServerEvent('tqrp_documents:CopyToPlayer', aPlayer, CURRENT_DOCUMENT)
...
RegisterCommand('docpublicos', function(source, args, rawCommand)
    close = false
	OpenNewPublicFormMenu()
end)

RegisterCommand('doclaborais', function(source, args, rawCommand)
    close = false
	OpenNewJobFormMenu()
end)

RegisterCommand('docguardados', function(source, args, rawCommand)
    close = false
	OpenMyDocumentsMenu()
end)
...
function DeleteDocument(aDocument)
    ...
    ESX.TriggerServerCallback('tqrp_documents:deleteDocument', function (cb)
        if cb == true then
            --remove form_close
...
function CreateNewForm(aDocument)
    ...
    PlayerData = ESX.GetPlayerData()
    ESX.TriggerServerCallback('tqrp_documents:getPlayerDetails', function (cb_player_details)
...
function ShowDocument(aPlayer)
        ...
        TriggerServerEvent('tqrp_documents:ShowToPlayer', aPlayer, CURRENT_DOCUMENT)
        CURRENT_DOCUMENT = nil
        CloseMenu()
end

RegisterNetEvent('tqrp_documents:viewDocument')
AddEventHandler('tqrp_documents:viewDocument', function( data )
...
RegisterNetEvent('tqrp_documents:copyForm')
AddEventHandler('tqrp_documents:copyForm', function( data )
...
function GetAllUserForms()
    ...
    ESX.TriggerServerCallback('tqrp_documents:getPlayerDocuments', function (cb_forms)
...
RegisterNUICallback('form_submit', function(data, cb)
    ...
    CloseMenu()
    ESX.TriggerServerCallback('tqrp_documents:submitDocument', function (cb_form)
        if cb_form ~= nil then
