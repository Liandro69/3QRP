...
ESX.RegisterServerCallback('tqrp_documents:submitDocument', function(source, cb, data)
...
ESX.RegisterServerCallback('tqrp_documents:deleteDocument', function(source, cb, id)
    ...
    MySQL.Async.execute('DELETE FROM user_documents WHERE id = @id AND owner = @owner',
    {
        ...
        if rowsChanged >= 1 then
            ...
            TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'success', text = 'Documento apagado' })
            cb(true)
        else
            --TriggerClientEvent('esx:showNotification', source, _U('document_delete_failed'))
            TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'success', text = 'Não conseguis te apagar' })
            cb(false)
        end
        end)
end)
...
ESX.RegisterServerCallback('tqrp_documents:getPlayerDocuments', function(source, cb)
...
ESX.RegisterServerCallback('tqrp_documents:getPlayerDetails', function(source, cb)
...
RegisterServerEvent('tqrp_documents:ShowToPlayer')
AddEventHandler('tqrp_documents:ShowToPlayer', function(targetID, aForm)

    TriggerClientEvent('tqrp_documents:viewDocument', targetID, aForm)

end)

RegisterServerEvent('tqrp_documents:CopyToPlayer')
AddEventHandler('tqrp_documents:CopyToPlayer', function(targetID, aForm)
            ...
            if id ~= nil then
                MySQL.Async.fetchAll('SELECT * FROM user_documents where id = @id', {['@id']=id}, function (result)
                    ...
                    if(result[1] ~= nil) then
                        db_form = result[1]
                        db_form.data = json.decode(result[1].data)
                        TriggerClientEvent('tqrp_documents:copyForm', _targetid, db_form)
