...
AddEventHandler("StreetRaces:joinRace_sv", function(index)
    ...
    if playerMoney >= amount then
        ...
        TriggerClientEvent("fh4speed:toogle", source, true)
        --TriggerClientEvent("carhud", source, false)
    else
        -- Insufficient money, send notification back to client
        local msg = "Sem guito pra te juntares a corrida"
...
AddEventHandler("StreetRaces:leaveRace_sv", function(index)
    ...
    local race = races[index]
    local players = race.players
    --TriggerClientEvent("carhud", source, true)
    TriggerClientEvent("fh4speed:toogle", source, false)
            ...
            if race.finishTime == 0 then
                ...
                for _, pSource in pairs(players) do
                    if pSource == source then
                        local msg = ("Ganhas te [%02d:%06.3f]"):format(timeMinutes, timeSeconds)
                        notifyPlayer(pSource, msg)
                    elseif config_sv.notifyOfWinner then
                        local msg = ("%s ganhou [%02d:%06.3f]"):format(getName(source), timeMinutes, timeSeconds)
                        notifyPlayer(pSource, msg)
                    end
                end
            else
                -- Loser, send notification to only the player
                local msg = ("Perdeste [%02d:%06.3f]"):format(timeMinutes, timeSeconds)
                notifyPlayer(source, msg)
            end
            TriggerClientEvent("fh4speed:toogle", source, false)
            --TriggerClientEvent("carhud", source, true)
...
AddEventHandler("StreetRaces:deleteRace_sv", function(name)
    ...
    if playerRaces[name] ~= nil then
        ...
        -- Send notification to player
        local msg = "Apagado " .. name
        notifyPlayer(source, msg)
    else
        local msg = "Não ha corridas com esse nome " .. name
        notifyPlayer(source, msg)
    end
end)
