...
function Spawn(Pcoords, Pped)	
	ESX.Game.SpawnVehicle(2218488798, Pcoords, 13, function(spawnedVehicle)
		-- save & set plate
...
function Process()
	...
	TriggerEvent("mythic_progbar:client:progress", {		 
		...
		duration = 6500,
		label = "A processar grãos de café....",
		useWhileDead = false,
		canCancel = true,
		controlDisables = {
	...
	while true do
		...
		if nearbyObject then
			if not isPickingUp and GetDisplayNameFromVehicleModel(GetEntityModel(veh)) == "TRACTOR2" then
				ESX.ShowHelpNotification("Pressiona L para ~g~Cultivares")
			end

			if IsControlJustReleased(0, 7) and not isPickingUp and GetDisplayNameFromVehicleModel(GetEntityModel(veh)) == "TRACTOR2" then
				ESX.TriggerServerCallback('caruby_farm:haveItem', function(haveItem)
					if haveItem then
						...
						ESX.ShowNotification("Tens que comprar uma Foice")
					end
				end)
			end
		else
...
--[[Citizen.CreateThread(function()	
	...
	while true do
		...
		if not IsPedInAnyVehicle(PlayerPedId()) then
			if GetDistanceBetweenCoords(coords, config.zones.cow.coords, true) < 3 then	
				ESX.ShowHelpNotification("Pressiona ~INPUT_CONTEXT~ to ~g~tirar o Trator")
				if not Tractor then	
