...
local local_hack_x = 1590.02 --Cordenadas alteradas para outra posição no meio do nada
local local_hack_y = -1978.49
local local_hack_z = 150.41 
...
function dinheiro_tick(variavel, variavel_2, variavel_3)
  for i=0, 1, 1 do
    ...
    if GetVehicleDoorAngleRatio(variavel, 2) > .1 or GetVehicleDoorAngleRatio(variavel, 3) > .1 and flag == 0 then
      ...
      dinheiro_carrof = CreateAmbientPickup(GetHashKey("PICKUP_MONEY_SECURITY_CASE"), GetOffsetFromEntityInWorldCoords(variavel, 0.0, -5.0, -0.3001), 0, 5569)
      TriggerServerEvent('mchacking:additem')
      ClearAllBlipRoutes()
      Wait(9000)
