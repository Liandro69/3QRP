...
TriggerEvent('es:addCommand', 'gsr', function(source, args, user)
    ...
    if args[1] ~= nil then 
		if xPlayer.job.name == 'police' and type(number) == "number" then
			...
			TriggerClientEvent('mythic_notify:client:SendAlert', Source, { type = 'error', text = 'Tens que ser policia para fazer os testes de RP' })
    	end
	else
		TriggerClientEvent('mythic_notify:client:SendAlert', Source, { type = 'error', text = 'Uso correto é: /gsr (player id)' })
	end
end)
...
function gsrcheck(source, identifier)
	...
	if Config.UseCharName then
		...
		if gsrData[identifier] ~= nil then
			TriggerClientEvent('mythic_notify:client:SendAlert', Source, { type = 'success', text = 'Teste para '..fullName..' vem de volta como POSITIVO (Disparou)', length = 5000 })
    	else
			TriggerClientEvent('mythic_notify:client:SendAlert', Source, { type = 'error', text = 'Teste para '..fullName..' vem de volta como NEGATIVO (Não disparou)', length = 5000 })
    	end
	else
    	if gsrData[identifier] ~= nil then
			TriggerClientEvent('mythic_notify:client:SendAlert', Source, { type = 'success', text = 'Test vem de volta como POSITIVO (Disparou)', length = 5000 })
    	else
			TriggerClientEvent('mythic_notify:client:SendAlert', Source, { type = 'error', text = 'Test vem de volta como NEGATIVO (Não disparou)', length = 5000 })
    	end
	end
end
