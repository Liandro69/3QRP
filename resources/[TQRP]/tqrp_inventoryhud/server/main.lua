...
TriggerEvent(
 ...
 --[[ ESX.RegisterServerCallback("tqrp_inventoryhud:getFastWeapons",function(source,cb)
...
--]]
--[[
RegisterServerEvent("tqrp_inventoryhud:changeFastItem")
AddEventHandler("tqrp_inventoryhud:changeFastItem",function(slot,weapon)
	local xPlayer = ESX.GetPlayerFromId(source)
	while not xPlayer do Citizen.Wait(10); ESX.GetPlayerFromId(source); end
	if slot ~= 0 then
		MySQL.Async.fetchAll(
...
ESX.RegisterServerCallback(
	"tqrp_inventoryhud:getPlayerInventory",
...
ESX.RegisterServerCallback(
		"tqrp_inventoryhud:getPlayerInventoryWeight",
		function(source,cb)
...
ESX.RegisterServerCallback('tqrp_inventoryhud:takePlayerItem', function(source, cb, item, count)
...
RegisterServerEvent("tqrp_inventoryhud:tradePlayerItem")
AddEventHandler(
	"tqrp_inventoryhud:tradePlayerItem",
...
ESX.RegisterServerCallback("suku:getShopItems", function(source, cb, shoptype)
	...
	for i=1, #itemResult, 1 do
		...
		if shoptype == "Hunt" then
			for _, v in pairs(Config.Shops.Hunt.Items) do
				if v.name == itemResult[i].name then
					table.insert(itemShopList, {
						type = "item_standard",
						name = itemInformation[itemResult[i].name].name,
		...
		if shoptype == "foodMachine" then
			for _, v in pairs(Config.Shops.FoodMachine.Items) do
				if v.name == itemResult[i].name then
					table.insert(itemShopList, {
						type = "item_standard",
						name = itemInformation[itemResult[i].name].name,
		...
		if shoptype == "drinkMachine" then
			for _, v in pairs(Config.Shops.DrinkMachine.Items) do
				if v.name == itemResult[i].name then
					table.insert(itemShopList, {
						type = "item_standard",
						name = itemInformation[itemResult[i].name].name,
...
local attach = {"flashlight", "grip", "scope", "skin", "suppressor"}
