...
--- Copy/Pasted by Daniel.
--- DateTime: 1/15/2020 3:14 PM
---
local shopData = nil
...
Citizen.CreateThread(function()
    while true do
        ...
        if IsInHuntShopZone(coords) or IsInRegularShopZone(coords) or IsInRobsLiquorZone(coords) or IsInIlegalShopZone(coords) or IsInYouToolZone(coords) or IsInPrisonShopZone(coords) or IsInWeaponShopZone(coords) or IsInPoliceShopZone(coords) then
            ...
            if IsInHuntShopZone(coords) then
                if IsControlJustReleased(0, Keys["E"]) then
                    if Licenses['hunt'] ~= nil or Licenses['hunt'] then
                        OpenShopInv("Hunt")
                        Citizen.Wait(2000)
                    else
                        exports['mythic_notify']:SendAlert('error', 'Necessitas de licensa de caça')
                    end
                end
            end
...
RegisterNetEvent("tqrp_inventoryhud:openShopInventory")
AddEventHandler("tqrp_inventoryhud:openShopInventory", function(data, inventory)
...
function IsInHuntShopZone(coords)
    HuntShop = Config.Shops.Hunt.Locations
    for i = 1, #HuntShop, 1 do
        if GetDistanceBetweenCoords(coords, HuntShop[i].x, HuntShop[i].y, HuntShop[i].z, true) < 1.5 then
            return true
        end
    end
    return false
...
Citizen.CreateThread(function()
    while true do
        ...
        if IsPedOnFoot(PlayerPedId()) then
            local distance = GetDistanceBetweenCoords(coords, Config.WeaponLiscence.x, Config.WeaponLiscence.y, Config.WeaponLiscence.z, true)
            if distance < 10 then
                DrawMarker(27, Config.WeaponLiscence.x, Config.WeaponLiscence.y, Config.WeaponLiscence.z, 0, 0, 0, 0, 0, 0, 0.5, 0.5, 1.0, 1, 157, 0, 155, false, false, 2, false, false, false, false)
                if distance < 3.0 then
                    DrawText3Ds(Config.WeaponLiscence.x, Config.WeaponLiscence.y, Config.WeaponLiscence.z + 1, "Pressiona [~g~E~w~] para acederes à loja")
                    if IsControlJustReleased(0, Keys["E"]) then
                        if Licenses['weapon'] == nil then
                            OpenBuyLicenseMenu()
                        else
                            exports['mythic_notify']:DoHudText('error', 'Já tens uma licença de arma!')
                        end
                        Citizen.Wait(2000)
                    end
                end
...
function OpenBuyLicenseMenu()
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_license',{
        title = 'Registar licença',
        elements = {
            { label = 'sim' ..' ($' .. Config.LicensePrice ..')', value = 'yes' },
            { label = 'não', value = 'no' },
        }
    },
            function (data, menu)
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if IsPedOnFoot(player) then
            ...
            for k, v in pairs(Config.Shops.Hunt.Locations) do
                local distance = GetDistanceBetweenCoords(coords, Config.Shops.Hunt.Locations[k].x, Config.Shops.Hunt.Locations[k].y, Config.Shops.Hunt.Locations[k].z, true)
                if distance < 10 then
                    DrawMarker(27, Config.Shops.Hunt.Locations[k].x, Config.Shops.Hunt.Locations[k].y, Config.Shops.Hunt.Locations[k].z, 0, 0, 0, 0, 0, 0, 0.5, 0.5, 1.0, 1, 157, 0, 155, false, false, 2, false, false, false, false)
                    if distance < 3.0 then 
                        DrawText3Ds(Config.Shops.Hunt.Locations[k].x, Config.Shops.Hunt.Locations[k].y, Config.Shops.Hunt.Locations[k].z + 1, "Pressiona [~g~E~w~] para acederes à loja")
                        near = true
                        break
                    end
                    near = true
