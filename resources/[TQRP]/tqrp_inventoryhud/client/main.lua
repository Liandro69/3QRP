        ...
        if not IsPlayerDead(PlayerId()) then
            ...
            if IsDisabledControlJustPressed(1, 37) and GetLastInputMethod(0) then
               -- if IsInputDisabled(0) then
                    openInventory()
                    if not HasAnimDictLoaded('pickup_object') then
...
function getPlayerWeight()
    Citizen.CreateThread(function()
        ESX.TriggerServerCallback("tqrp_inventoryhud:getPlayerInventoryWeight", function(cb)
...
function loadItems()
    Citizen.CreateThread(function()
        ESX.TriggerServerCallback("tqrp_inventoryhud:getPlayerInventory", function(data)
            items = {}
            fastItems = {}
    ...
    if closestPlayer ~= -1 and closestDistance <= 3.0 then
        ...
        if IsEntityPlayingAnim(searchPlayerPed, 'random@mugging3', 'handsup_standing_base', 3) or IsEntityDead(searchPlayerPed) or GetEntityHealth(searchPlayerPed) <= 0 or IsEntityPlayingAnim(searchPlayerPed, "mp_arresting", "idle", 3) or IsEntityPlayingAnim(searchPlayerPed, "mp_arrest_paired", "crook_p2_back_right", 3) then
            exports['mythic_progbar']:Progress({
                ...
                if not status then
                    TriggerEvent("tqrp_inventoryhud:openPlayerInventory", GetPlayerServerId(closestPlayer))
...
AddEventHandler('tqrp_inventoryhud_inventoryhud:doClose', function(...) 
    closeInventory(...); 
end)
