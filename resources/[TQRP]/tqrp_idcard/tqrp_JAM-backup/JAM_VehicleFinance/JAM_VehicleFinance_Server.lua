...
ESX.RegisterServerCallback('JAM_VehicleFinance:RepoVehicle', function(source,cb,vehicleProps)
	...
	for k,v in pairs(data) do
		...
		if v.financetimer == 0 then
			MySQL.Async.execute('UPDATE FROM owned_vehicles SET state = 3 WHERE plate = @plate', {['@plate'] = vehicleProps.plate})
			TriggerEvent("tqrp_base:serverlog", "[REMOVED CAR] FROM", GetPlayerName(source), GetCurrentResourceName())
		end
	end	
	cb(cbData)
end)
