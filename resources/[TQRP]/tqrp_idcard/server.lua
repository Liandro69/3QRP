...
-- Open ID card
RegisterServerEvent('tqrp_idcard:open')
AddEventHandler('tqrp_idcard:open', function(ID, targetID, type)
		...
		if (user[1] ~= nil) then
			...
			function (licenses)
				...
				if show then
					...
					TriggerClientEvent('tqrp_idcard:open', _source, array, type)
				else
					TriggerClientEvent('esx:showNotification', _source, "Não tens essa licensa..")
				end
			end)
		end
	end)
