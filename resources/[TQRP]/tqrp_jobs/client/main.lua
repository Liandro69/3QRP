...
Citizen.CreateThread(function()
	...
	--[[while true do
		for i,v in pairs(Config.NPCs) do
			if #(GetEntityCoords(PlayerPedId()) - vector3(v.coords.x, v.coords.y, v.coords.z)) <= 30 then
				if not DoesEntityExist(peds[i]) then
					peds[i] = X.SpawnPed(v.model, v.coords)
				end
			else
...
Local = {
	enterWarehouse  = "Pressiona E para entrar no armazém.",
	cancelCharge    = "Pressiona E para cancelar a carga",
	warehouse       = "Armazém",
	wood            = "Madeira",
	food            = "Alimentos",
	mine            = "Mina",
	...
	getTruck        = "Pressiona E  para colher uma carga de ",
	truck           = "Camião",
	van             = "Carrinha",
	youSelect       = "Escolheste colher",
	inLabel         = "em",
	...
	haveToDelivery  = "Tens que entregar a carga primeiro.",
	...
	deliveryTruck   = "Pressiona E para entregar el trailer.",
	...
	upToBack        = "Já podes regressar ao armazém para cobrar",
	succesfulDelivery = "Já fizeste a entrega correctamente",
	getBox            = "Pressiona E para carregar a carrrinha",
	...
	ready             = "Pronto",
	pressButton       = "Pressiona ~r~E para falar",
	wateringcan       = "Regador",
	...
	salinity          = "~c~Aditivos",
	chemists          = "~g~Químicos",
	pollution         = "Contaminação",
	pointWater        = "Vinhas",
	dirtyWater        = "~o~Uva Madura",
	electricityDown   = "~b~Baixar aditivos com sistema de filtragem",
	dam               = "Barragem",
	allow             = "~g~Disponível",
	notAllow          = "~r~ Indisponível",
	noActionAllow     = "Essa ação não é permitida",
	catCall           = "~c~Chama o Ramiro pra reduzir a puluição",
	greenpeaceAcept   = "~c~Reza para que o Sto.Antonio faça chover",
	youAreCapitalist  = "Agora queres que te ajude? Sócio capitalista de merda, embora daqui.",
	capitalistAccept  = "Queres unir-te ao sistema? Só tens que apoiar a minha causa. VIVA AO PARTIDO RENOVADOR!",
	youAreGreenPeacer = "Tu queres montar agora una empresa? Olá? Apoia o meio ambiente? Por favor...",
	toxic1            = "Abrir a válvula para adicionar químicos na água",
	salinity1         = "Aumentar indução de aditivos",
	salinity2         = "Retrai um pouco os aditivos",
	pollution1        = "Vigiar os bandidos que querem foder a vinha",
	pollution2        = "Adiciona veneno na agua que rega a vinha",
	chemist1          = "Adora o deus salva-vidas",
	pollution3        = "Corrigir o filtro de contaminação",
	salinity3         = "Parece haver um rato a comer as uvas, desculpa, és tu. Corrige o filtro de aditivos.",
	youAreDoingSomething = "Para o que estás a fazer!",
	noWaterPointsNearby = "Não há pontos de recolha de vinhaça nas proximidades",
	youAreRecolecting = "Estás a coletar vinhaça",
	waterBottle       = "Garrafa de vidro 22$",
	bottleShop        = "Loja de Garrafas de Vidro",
	washBottles       = "Queres limpar as uvas?",
	washSaltBottles   = "Tirar os aditivos das uvas?",
	washBottle        = "Limpar uvas",
	packageWater      = "Empacotar Vinhaça",
	package           = "Sr.Ramiro das Vinhaças",
...
Config.Blips = {
    ...
    woodProcesser = {title=Local.convertWood, colour=56, id=486, x = 1213.56, y = -1262.53, z = 35.23},
    ...
    water = {title=Local.pointWater, colour=29, id=486, x = -1849.51, y = 2178.52, z = 108.93},
    dam = {title=Local.dam, colour=29, id=670, x = 1660.05, y = -1.34, z = 173.77},
    waterWashBottle = {title=Local.washBottle, colour=29, id=486, x = 822.62, y = -2339.19, z = 29.33},
...
Config.NPCs = {
	...
	{
		name = 'OpenProcesserMenu', --Processar 
		coords = {
			x = 822.62,
			y = -2339.19,
			z = 29.33,
...
X.SpawnPeds = function()
	for i,v in pairs(Config.NPCs) do
		local model           = (tonumber(v.model) ~= nil and tonumber(v.model) or GetHashKey(v.model))
		RequestModel(model)
	...
	Citizen.CreateThread(function()
		while true do
			for i,v in pairs(Config.NPCs) do
				...
				local z = v.coords.z
				local h = v.coords.h
				if #(GetEntityCoords(PlayerPedId()) - vector3(x, y, z)) <= 30 then 
					if not DoesEntityExist(ped[i]) then
						ped[i] = CreatePed(5, model, x, y, z, h, false, false)
						FreezeEntityPosition(ped[i],true)
					end
				else
					if DoesEntityExist(ped[i]) then
						DeleteEntity(ped[i])
					end
				end
			end
...
X.OpenSetMenu = function(id)
	...
	ESX.TriggerServerCallback('tm1_stores:getWarehouse', function(warehouse)
	    ...
	    table.insert(elements, {value = "water", valueLabel = Local.water,  material = X.LastShop.materials.water, maxMaterial = X.LastShop.maxMaterials.water, label = "<span style='color: cyan;'>Vinhaça</span><b> : "..X.LastShop.materials.water.."/"..X.LastShop.maxMaterials.water.."</b>"})
