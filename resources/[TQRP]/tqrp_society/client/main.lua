...
function RefreshBussHUD()
	...
	if ESX.PlayerData.job.grade_name == 'boss' then
		...
		ESX.TriggerServerCallback('tqrp_society:getSocietyMoney', function(money)
			UpdateSocietyMoneyHUDElement(money)
		end, ESX.PlayerData.job.name)
	end
end

RegisterNetEvent('tqrp_addonaccount:setMoney')
AddEventHandler('tqrp_addonaccount:setMoney', function(society, money)
...
function EnableSocietyMoneyHUDElement()
	...
	TriggerEvent('tqrp_society:toggleSocietyHud', true)
end
...
function DisableSocietyMoneyHUDElement()
	...
	TriggerEvent('tqrp_society:toggleSocietyHud', false)
end
...
function UpdateSocietyMoneyHUDElement(money)
	...
	TriggerEvent('tqrp_society:setSocietyMoney', money)
end
...
function OpenBossMenu(society, close, options)
	...
	ESX.TriggerServerCallback('tqrp_society:isBoss', function(result)
		isBoss = result
	end, society)
	...
	if options.bills then
		table.insert(elements, {label = "Faturação", value = 'bills'})
	end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'boss_actions_' .. society, {
		...
		if data.current.value == 'withdraw_society_money' then
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'withdraw_society_money_amount_' .. society, {
				...
				if amount == nil then
					TriggerEvent("pNotify:SendNotification", {text = "<span style='font-weight: 300'> " .. _U('invalid_amount') .. " </span>",
						layout = "topRight",
						timeout = 2000,
						progressBar = false,
						type = "error",
					...
					menu.close()
					TriggerServerEvent('tqrp_society:withdrawMoney', society, amount)
				end
		...
		elseif data.current.value == 'deposit_money' then
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'deposit_money_amount_' .. society, {
				...
				if amount == nil then
					...
					menu.close()
					TriggerServerEvent('tqrp_society:depositMoney', society, amount)
				end
		...
		elseif data.current.value == 'bills' then
			...
			ESX.TriggerServerCallback('tqrp_billing:getSpecialSocietyBills', function(results)
				elements = {}
				for i=1, #results, 1 do
					table.insert(elements, {label =   results[i].label .. " | " ..  results[i].day .. "/" .. results[i].month .. " | " .. results[i].amount .. "€" ..  " | " .. results[i].firstname .. " " .. results[i].lastname , value = i})
				end
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'billing',
				{
					title    = "Faturas em espera",
					align    = 'bottom-right',
					elements = elements
				}, function(data, menu)
				end, function(data, menu)
		...
		elseif data.current.value == 'wash_money' then
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'wash_money_amount_' .. society, {
				...
				if amount == nil then
					...
					menu.close()
					TriggerServerEvent('tqrp_society:washMoney', society, amount)
				end
...
function OpenEmployeeList(society)
	...
	ESX.TriggerServerCallback('tqrp_society:getEmployees', function(employees)
		...
		ESX.UI.Menu.Open('list', GetCurrentResourceName(), 'employee_list_' .. society, elements, function(data, menu)
			...
			elseif data.value == 'fire' then
				TriggerEvent("pNotify:SendNotification", {text = "<span style='font-weight: 300'> " .. _U('you_have_fired', employee.name) .. " </span>",
					layout = "topRight",
					timeout = 2000,
					progressBar = false,
					type = "sucess",
				...
				ESX.TriggerServerCallback('tqrp_society:setJob', function()
					OpenEmployeeList(society)
...
function OpenRecruitMenu(society)
	...
	ESX.TriggerServerCallback('tqrp_society:getOnlinePlayers', function(players)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'recruit_' .. society, {
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'recruit_confirm_' .. society, {
				...
				if data2.current.value == 'yes' then
					TriggerEvent("pNotify:SendNotification", {text = "<span style='font-weight: 300'> " .. _U('you_have_fired', data.current.name) .. " </span>",
						layout = "topRight",
						timeout = 2000,
						progressBar = false,
						type = "sucess",
...
function OpenPromoteMenu(society, employee)
	...
	ESX.TriggerServerCallback('tqrp_society:getJob', function(job)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'promote_employee_' .. society, {
			...
			menu.close()
			TriggerEvent("pNotify:SendNotification", {text = "<span style='font-weight: 300'> " .. _U('you_have_promoted', employee.name, data.current.label) .. " </span>",
				layout = "topRight",
				timeout = 2000,
				progressBar = false,
				type = "sucess",
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'manage_grades_' .. society, {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'manage_grades_amount_' .. society, {
				...
				elseif amount > Config.MaxSalary then
					TriggerEvent("pNotify:SendNotification", {text = "<span style='font-weight: 300'> " .. _U('invalid_amount_max') .. " </span>",
						layout = "topRight",
						timeout = 2000,
						progressBar = false,
						type = "error",
					...
					ESX.TriggerServerCallback('tqrp_society:setJobSalary', function()
						OpenManageGradesMenu(society)
...
AddEventHandler('tqrp_society:openBossMenu', function(society, close, options)
