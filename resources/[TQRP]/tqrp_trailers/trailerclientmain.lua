...
Citizen.CreateThread(function()
	while true do
		...
		local asd, TrailerHandle = GetVehicleTrailerVehicle(GetVehiclePedIsIn(myPed, 1))

		if TrailerHandle ~= 0 and TrailerHandle ~= false then
			oldtrailer = TrailerHandle
			oldtrailermodel = GetEntityModel(oldtrailer)
		elseif oldtrailermodel ~= nil then
			if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), GetEntityCoords(oldtrailer)) >= 10.0 then
				oldtrailermodel = nil
			end
		end
		
		if oldtrailermodel == 712162987 then -- small trailer
			...
			if currentVehicle == 0 then -- a pied
				if oldtrailer ~= 0 then
					...
					if dist < 5 then
						...
						DrawMarker(21, coords.x, coords.y, coords.z, 0, 0, 0, 0, 0, 0, 1.0, 1.0, 1.0, 255, 0, 0, 200, 0, 0, 0, 0)
								...
								if CurrentlyTowedVehicle7 == nil then
									if Config.Cartrailer[modelhash] ~= nil then
										...
										elseif (Config.SmallTrailerTowListOnly == true) then 
											ESX.ShowNotification('Este veículo não pode ser atrelado.')
										end
									end	
									if (Config.SmallTrailerTowListOnly == false) then 
										CurrentlyTowedVehicle7 = clostestvehicle
