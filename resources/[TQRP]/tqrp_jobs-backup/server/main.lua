...
Local = {
	...
	removeSalinity = "Tiraste a salinidade da água",
	removePollution = "Retiraste a contaminação da água",
	addChemists = "Adicionaste químicos à água",
	addSalinity = "Adicionaste salinidade à água",
	addPollution = "Adicionaste contaminação à água",
	noBottles = "Não tens garrafas para limpar",
	goodWashing = "Limpaste corretamente as garrafas",
...
Config.BasePrices = {
 	bottleWater_package = 15,
 	blueberry_package = 50,
 	tomato_package = 50,
 	lingot_carbon = 30,
 	lingot_iron =50,
 	lingot_silver = 14,
...
AddEventHandler('tm1_stores:craft',function(item)
	...
	if xPlayer then
		...
		if quantityMineral.count >= 20 then
			if xPlayer.canCarryItem("lingot_"..item,5)	then
				xPlayer.removeInventoryItem(item.."_piece",20)
...
AddEventHandler('tm1_stores:giveWood', function(wood)
	local xPlayer  = ESX.GetPlayerFromId(source)
	if xPlayer then
		if xPlayer.canCarryItem(wood.."_wood", 5)	then
			xPlayer.addInventoryItem(wood.."_wood", 5)
		else
			TriggerClientEvent('mythic_notify:client:SendAlert', source, {type = 'error', text = "Não tens espaço suficiente", length = 5500})
		end
	end
...
AddEventHandler('tm1_stores:craftWood',function(item)
	...
	if xPlayer then
		...
		if quantityWood.count >= 20 then
			if xPlayer.canCarryItem(item.."_wood",10)	then
