...
AddEventHandler('tqrp_phone:server:CharacterSpawned', function()
    ...
    TriggerClientEvent('tqrp_phone:client:SetSettings', src, settings.settings)
    TriggerClientEvent('tqrp_phone:client:SetupData', src, { { name = 'settings', data = settings.settings } })
end)
...
RegisterServerEvent('tqrp_phone:server:SaveData')
AddEventHandler('tqrp_phone:server:SaveData', function(data)
        local src = source
        local char = ESX.GetPlayerFromId(src)
