RegisterServerEvent('tqrp_phone:server:irc')
AddEventHandler('tqrp_phone:server:irc', function()
local src = source
    ...
    Citizen.CreateThread(function()
        local myChannels = {}
         exports['ghmattimysql']:execute('SELECT channel FROM phone_irc_channels WHERE identifier = @identifier', { ['identifier'] = player.identifier }, function(channels) 
            TriggerClientEvent('tqrp_phone:client:SetupData', src, { { name = 'irc-channels', data = channels } })
        end)
    end)
end)

ESX.RegisterServerCallback('tqrp_phone:server:IRCJoinChannel', function(source, cb, data)
    local src = source
    local player = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('tqrp_phone:server:IRCLeaveChannel', function(source, cb, data)
    local src = source
    local player = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('tqrp_phone:server:IRCGetMessages', function(source, cb, data)
    local src = source
    local player = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('tqrp_phone:server:IRCNewMessage', function(source, cb, data)
    local src = source
    local player = ESX.GetPlayerFromId(source)
                        ...
                        exports['ghmattimysql']:execute('SELECT * FROM phone_irc_channels WHERE channel = @channel', { ['channel'] = data.channel }, function(data2)
                            for k, v in ipairs(data2) do
                                    ...
                                    local tPlayer = v.identifier
                                        TriggerClientEvent('tqrp_phone:client:ReceiveNewIRCMessage', otherplayer, { channel = data.channel, message = data.message })
                            end
                        end)
                    end)
                end
