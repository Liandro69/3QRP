    ...
    $.post(ROOT_ADDRESS + '/DeleteCallRecord', JSON.stringify({
        ...
        if(status){
            elem.parent().parent().fadeOut('normal', function() {
                var history = GetData('history');
                history.splice(data.index, 1);
                StoreData('history', history);
                M.toast({html: 'Registo apagado'});
                //RefreshApp();
            });
        } else {
...
function CreateCall(number, nonStandard, receiver) {    
    $.post(ROOT_ADDRESS + '/CreateCall', JSON.stringify({
        ...
        if (status > 0) {
            ...
            M.toast({html:'Não podes ligar para o teu número'})   
        }else if (status == -3) {
            M.toast({html:'Número ocupado'})
        } else {
            M.toast({html:'Número não disponível'})
        }
    });
}
...
function SetupCallContacts() {
    ...
    $.each(contacts, function(index, contact) {
        if(contact.avatar != null){
            $('[data-section=contacts').find('.contacts-list').append('<div class="phone-contact"><div style="background-image: url('+contact.avatar+');" class="phone-avatar other-' + contact.name[0].toString().toLowerCase() + '"> </div>' + contact.name + '<div class="call-actions"><i class="fas fa-phone-volume call-action-call"></i><i class="fas fa-sms call-action-text"></i></div></div>');
        }
        else{
