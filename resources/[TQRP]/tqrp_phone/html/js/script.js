...
$(document).ready(function() {
    ...
    $.post('http://tqrp_phone/getWeather', JSON.stringify({}));
    ...
    document.onkeyup = function(data) {
        ...
        if (data.which == 27) {
            $.post('http://tqrp_phone/close', JSON.stringify({}));
        }
    }
        ...
        if (action !== undefined) {
            switch (action) {
                case "yellow-pages-delete":
                    $.post('http://tqrp_phone/deleteYP', JSON.stringify({}));
                    break;
                case "newPostSubmit":
                    e.preventDefault();
                    $.post('http://tqrp_phone/newPostSubmit', JSON.stringify({
                ...
                case "getCallHistory":
                    if (callStates[currentCallState] === "isCallInProgress" && currentContainer !== "incoming-call")
                        ...
                        $.post('http://tqrp_phone/' + action, JSON.stringify({}));
                    break;
                default:
                    $.post('http://tqrp_phone/' + action, JSON.stringify({}));
                    break;
    ...
    window.addEventListener('message', function(event) {
        ...
        switch (item.openSection) {
            ...
            case "photo_twatter":
                ...
                if (item.photo) {
                    $("#twat-modal").find('img').attr("src", item.photo);
                    $("#twat-modal").find('img').show();
                } else {
                    $("#twat-modal").find('img').hide();
                }
                break;
            ...
            case "racing:hud:update":
                switch (item.hudState) {
                    ...
                    case "finished":
                        ...
                        drawRaceStats();
                        $.post('http://tqrp_phone/race:completed', JSON.stringify({
...
function openBrowser(url) {
    ...
    closePhoneShell();
    $.post('http://tqrp_phone/btnCamera', JSON.stringify({}));
    $("#browser").fadeIn(300);
}
...
function addVehicles(vehicleData, showCarPayments) {
    ...
    for (let vehicle of Object.keys(vehicleData)) {
        ...
        if (vehicleData[vehicle].garage == "Garagem OUT"){
            vehicleData[vehicle].garage = "FORA";
            carBGColor = "red";
        }
            else
        {
                    ...
                    <div class="row">
                        <div class="col s12"> 
                            <ul class="collection">
                                ...
                                <li class="collection-item"><i class="fas fa-car-crash"></i> ${vehicleData[vehicle].bodyPercent}% Visual</li>
                                <li class="collection-item"><i class="fas fa-hourglass-half"></i> ${vehicleData[vehicle].payments == 0 ? 'Nenhum pagamento.' : (vehicleData[vehicle].lastPayment) + ' dias para pagar.'}</li>
                                `
        if (vehicleData[vehicle].payments != 0) {
            vehicleElement += `
                                <li class="collection-item"><i class="fas fa-dollar-sign"></i> <span class="car-payment-due">${vehicleData[vehicle].amountDue}</span> montante a pagar.</li>
                                `
        }
        vehicleElement += `
                            </ul>
        ...
        if (vehicleData[vehicle].canSpawn)
            vehicleElement += `<button class="waves-effect waves-light btn-small garage-spawn" data-plate="${vehicleData[vehicle].plate}"><i class="fas fa-magic"></i> Pedir</button> `

        if (vehicleData[vehicle].payments > 0 && vehicleData[vehicle].amountDue > 0)
            vehicleElement += `<button class="waves-effect waves-light btn-small red garage-pay" data-plate="${vehicleData[vehicle].plate}"><i class="fas fa-hand-holding-usd"></i> Pagar</button> `

        vehicleElement += `<button class="waves-effect waves-light btn-small garage-track" data-plate="${vehicleData[vehicle].plate}"><i class="fas fa-map-marker-alt"></i> LOCALIZAR</button>
                        </div>
                    </div>
                </div>
            </li>
...
function addAccountInformation(accountInfo) {
    if (accountInfo) {
        ...
        $('.account-information-secondary-job').text(accountInfo.time ? accountInfo.time  : 0);
        $('.account-information-time').text(accountInfo.time ? accountInfo.time  : 0);
        $('.account-information-phone').text(accountInfo.phone ? accountInfo.phone  : 0);
        /* .split('<br>') */
        // let licensesObject = {}
        ...
        /*let licenseTable =
            ...
            <tbody>
            `
        for (const key of Object(accountInfo.licenses)) {
            licenseTable += `<tr>
                                <td>${key["label"]}</td>
...
function addTweets(tweets, myHandle) {
    ...
    if (tweets && Object.keys(tweets).length > 0) {
        for (let message of tweets) {
            if (message && message.handle && message.message && message.photo == null) {
                var twat = message.message;
                if (twat !== "") {
                    var twatEntry = $(`<div class="row no-padding">
                            <div class="col s12">
                                <div class="card white lighten-1 twat-card">
                                    <div class="card-content black-text twatter-content">
                                    ...
                                    <div class="card-action" style="padding-top: 8px;padding-right: 16px;padding-bottom: 8px;padding-left: 16px;">
                                        <span data-poster="${message.handle}" class="twat-reply black-text"><i class="fas fa-reply fa-1x"></i></span>
                                        <span class="right black-text" aria-label="${moment.utc(message.time).local().calendar(null, calendarFormatDate)}" data-balloon-pos="down">${moment.utc(message.time).local().fromNow()}</span>
                                    </div>
                                 </div> 
                            </div>
                         </div>`);
                            ...
                            <div class="col s12">
                                <div class="card white lighten-1 twat-card">
                                    <div class="card-content black-text twatter-content">
                                         <span class="card-title twatter-title">${message.handle}</span>
                                         <p>${twat}</p>
                                         <img id="twat-photo" src="${photo}">
                                    </div>
                                    <div class="card-action" style="padding-top: 8px;padding-right: 16px;padding-bottom: 8px;padding-left: 16px;">
                                        <span data-poster="${message.handle}" data-photo="${photo}" class="twat-reply black-text"><i class="fas fa-reply fa-1x"></i></span>
                                        <span class="right black-text" aria-label="${moment.utc(message.time).local().calendar(null, calendarFormatDate)}" data-balloon-pos="down">${moment.utc(message.time).local().fromNow()}</span>
                                    </div>
                                 </div> 
                            </div>
            ...
            <div class="col s12">
                <div class="card yellow darken-1 yellow-page-entry">
                    ...
                    <div class="card-action" style="padding-top: 8px;padding-right: 16px;padding-bottom: 8px;padding-left: 16px;font-size:14px">
                        <div class="row no-padding">
                            <div class="col s6">
                                <span aria-label="Chamar" data-balloon-pos="up-left" data-number="${item.phoneNumber}" class="yellow-pages-call"><i class="fas fa-phone-alt fa-1x"></i> ${item.phoneNumber}</span>
                            </div>
...
function addMessage(item) {
    ...
    element.click(function () {
        $.post('http://tqrp_phone/messageRead', JSON.stringify({ sender: item.sender, receiver: item.receiver, displayName: item.msgDisplayName }));
    });
...
function addMessageOther(item) {
    ...
    element.id = item.id;
    element.click(function () {
        $.post('http://tqrp_phone/messageRead', JSON.stringify({ sender: item.sender, receiver: item.receiver, displayName: receiver, clientPhone: item.clientNumber }));
    });
    $(".messages-entries").prepend(element);
}
...
function addContact(item) {
    if (contactList.some(function (e) 
    { 
...
function updateSettings()
{
    switch (currentSettingWindow) {
        case "tokovoip":
            ...
            break;
        case "control":
            $.post('http://tqrp_phone/settingsUpdateToko', JSON.stringify({tag: "controlUpdate",controls: currentBinds}));
            break;
        case "browser":
            break;
        default:
...
function ResetSettings()
{
    switch (currentSettingWindow) {
        case "tokovoip":
            $.post('http://tqrp_phone/settingsResetToko', JSON.stringify());
            break;
        case "control":
            $.post('http://tqrp_phone/settingsResetControls', JSON.stringify());
            break;
        case "browser":
            break;
        default:
...
async function updateTokoSettings()
{
    ...
    await delayedLog();
    $.post('http://tqrp_phone/settingsUpdateToko', JSON.stringify({
        tag: "settings",
...
$('#racing-create-form').on('reset', function (e) {
    $.post('http://tqrp_phone/racing:map:cancel', JSON.stringify({}));
});
...
function SetMute(status) {
    ...
    if (status) {
        ...
        $.post('http://tqrp_phone/btnMute', JSON.stringify({}));
...
$('#racing-start-tracks').on('change', function (e) {
    ...
    if(maps[selectedMap] !== undefined) {
        $.post('http://tqrp_phone/racing:map:removeBlips', JSON.stringify({}));
        $.post('http://tqrp_phone/racing:map:load', JSON.stringify({ id: selectedMap}));
...
$('#racing-start').submit(function (e) {
    e.preventDefault();
    $.post('http://tqrp_phone/racing:event:start', JSON.stringify({
...
$('#racing-create-form').submit(function (e) {
    e.preventDefault();
    $.post('http://tqrp_phone/racing:map:save', JSON.stringify({
...
$("#real-estate-sell-form").submit(function (e) {
    e.preventDefault();
    $.post('http://tqrp_phone/btnAttemptHouseSale', JSON.stringify({
...
$('#real-estate-transfer-form').submit(function (e) {
    e.preventDefault();
    $.post('http://tqrp_phone/btnTransferHouse', JSON.stringify({
...
$("#group-manage-pay-form").submit(function (e) {
    ...
    $.post('http://tqrp_phone/payGroup', JSON.stringify({
...
$("#group-manage-rank-form").submit(function (e) {
    e.preventDefault();
    $.post('http://tqrp_phone/promoteGroup', JSON.stringify({
        gangid: escapeHtml($(".group-manage-company-name").data('group-id')),
...
$("#group-manage-bank-form").submit(function (e) {
    ...
    $.post('http://tqrp_phone/bankGroup', JSON.stringify({
        gangid: escapeHtml($(".group-manage-company-name").data('group-id')),
...
$("#group-tasks-assign-modal-form").submit(function (e) {
    ...
    $.post('http://tqrp_phone/btnGiveTaskToPlayer', JSON.stringify({
...
$("#contacts-form").submit(function (e) {
    ...
    $.post('http://tqrp_phone/newContactSubmit', JSON.stringify({
        name: clean,
...
$("#stock-form").submit(function (event) {
    event.preventDefault();
    $.post('http://tqrp_phone/stocksTradeToPlayer', JSON.stringify({
...
$("#twat-form").submit(function (event) {
    event.preventDefault();
    if( $("#twat-modal").find('img').css('display') == 'none' )
    {
        $.post('http://tqrp_phone/newTwatSubmit', JSON.stringify({
        ...
        $.post('http://tqrp_phone/newTwatSubmit', JSON.stringify({
            twat: escapeHtml($("#twat-form #twat-body").val()),
            photo: $("#twat-modal").find('img').attr('src'),
            time: moment.utc()
        })).then($("#twat-modal").find('img').attr("src", null));
    }
...
$("#call-form").submit(function (event) {
    event.preventDefault();
    $.post('http://tqrp_phone/callContact', JSON.stringify({
        name: '',
...
$("#new-message-form").submit(function (event) {
    ...
    $.post('http://tqrp_phone/newMessageSubmit', JSON.stringify({
    ...
    switch (currentContainer) {
        case "message":
            setTimeout(function () {
                ...
                $.post('http://tqrp_phone/messageRead', JSON.stringify({ sender: sender, receiver: receiver, displayName: displayName }));
            }, 300);
            break;
        case "messages":
            setTimeout(function () {
                $.post('http://tqrp_phone/messages', JSON.stringify({}));
            }, 300);
            break;
    }
...
$('.racing-map-delete-confirm').click(function () {  
    ...
    $.post('http://tqrp_phone/racing:map:delete', JSON.stringify({ id: raceMap }));
    $('.racing-delete-confirm').fadeOut(150)
...
$('#real-estate-evict-modal-accept').click(function () {
    $.post('http://tqrp_phone/btnEvictHouse', JSON.stringify({}));
...
$('.twatter-toggle-notification').click(function () {
    ...
    $.post('http://tqrp_phone/btnNotifyToggle', JSON.stringify({}));
});

$('.account-information-toggle-pager').click(function () {
    $.post('http://tqrp_phone/btnPagerToggle', JSON.stringify({}));
...
$('.racing-entries').on('click', '.racing-entries-join', function () {
    $.post('http://tqrp_phone/racing:event:join', JSON.stringify({ identifier: $(this).data('id') }));
});

$('.keys-entries').on('click', '.manage-keys', function () {
    $.post('http://tqrp_phone/retrieveHouseKeys', JSON.stringify({}));
});

$('.keys-entries').on('click', '.remove-shared-key', function(e) {
    $.post('http://tqrp_phone/removeSharedKey', JSON.stringify({
...
$('.manage-keys-entries').on('click', '.manage-keys-remove', function () {
    $.post('http://tqrp_phone/removeHouseKey', JSON.stringify({
...
$("#tweetar" ).click(function() {
    $("#twat-modal").find('img').css("display", "none");
});
...
$('.twatter-entries').on('click', '.twat-reply', function () {
    ...
    if (!$(this).data('photo')) {
        ...
        $("#twat-modal").find('img').attr("src", $(this).data('photo'));
        $("#twat-modal").find('img').show();
    }
...
$('.group-tasks-entries').on('click', '.group-tasks-track', function () {
    $.post('http://tqrp_phone/trackTaskLocation', JSON.stringify({ TaskIdentifier: $(this).data('id') }));
});

$('.delivery-job-entries').on('click', '.delivery-job-accept', function (e) {
    $.post('http://tqrp_phone/selectedJob', JSON.stringify({ jobType: $(this).data('job-type'), jobId: $(this).data('job-id') }));
});
...
$('.garage-entries').on('click', '.garage-spawn', function (e) {
    e.preventDefault();
    $.post('http://tqrp_phone/vehspawn', JSON.stringify({ vehplate: $(this).data('plate') }));
    $.post('http://tqrp_phone/btnGarage', JSON.stringify({}));
});

$('.garage-entries').on('click', '.garage-track', function () {
    $.post('http://tqrp_phone/vehtrack', JSON.stringify({ vehplate: $(this).data('plate') }));
});

$('.garage-entries').on('click', '.garage-pay', function (e) {
    $.post('http://tqrp_phone/vehiclePay', JSON.stringify({ vehiclePlate: $(this).data('plate') }));
    setTimeout(function () {
        $.post('http://tqrp_phone/btnGarage', JSON.stringify({}));
    }, 1500);
});
...
$('.gps-entries, .keys-entries').on('click', '.gps-location-click', function () {
    $.post('http://tqrp_phone/loadUserGPS', JSON.stringify({ house_id: $(this).data('house-id'), house_type: $(this).data('house-type') }));
})

$('.contacts-entries, .call-history-entries').on('click', '.btn-contacts-call', function () {
    $.post('http://tqrp_phone/callContact', JSON.stringify({ name: $(this).data('name'), number: $(this).data('number') }));
});
...
$('.contacts-entries-wrapper').on('click', '.btn-contacts-remove', function () {
    ...
    $('#confirm-modal-question').text(`Tem a certeza que pretende apagar: ${$(this).data('name')}?`);
});

$('#confirm-modal-accept').click(function (event) {
    $.post('http://tqrp_phone/removeContact', JSON.stringify({ name: $(this).data('name'), number: $(this).data('number') }));
