...
function MakeDataStore(plate)
  ...
  local dataStore = CreateDataStore(plate, owned, data)
  SharedDataStores[plate] = dataStore
  TriggerEvent("tqrp_base:serverlog", "CreateDataStore ["..plate.."]", "SERVER", GetCurrentResourceName())
  MySQL.Async.execute(
...
function GetSharedDataStore(plate)
  if SharedDataStores[plate] == nil and plate~= nil then
    TriggerEvent("tqrp_base:serverlog", "MakeDataStore ["..plate.."]", "SERVER", GetCurrentResourceName())
    MakeDataStore(plate)
  end
  return SharedDataStores[plate]
end

AddEventHandler("tqrp_trunk:getSharedDataStore", function(plate, cb)
  local shared = GetSharedDataStore(plate)
  if shared ~= nil then
