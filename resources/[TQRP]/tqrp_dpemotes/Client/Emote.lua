...
local started = false
local stage = {
	[1] = { done = false, dict = "missminuteman_1ig_2", anim = "handsup_enter", msg = "[~g~E~w~] Ajoelhar", key = 38, wait = 500},
	[2] = { done = false, dict = "random@arrests", anim = "idle_2_hands_up", msg = "[~g~E~w~] Colocar mãos na cabeça", key = 38, wait = 4000},
	[3] = { done = false, dict = "random@arrests@busted", anim = "enter", msg = "[~g~E~w~] Tirar mãos na cabeça", key = 38, wait = 700},
	[4] = { done = false, dict = "random@arrests@busted", anim = "exit", msg = "[~g~E~w~] Levantar", key = 38, wait = 1500},
	[5] = { done = false, dict = "random@arrests", anim = "kneeling_arrest_get_up", msg = "", key = 38, wait = 1000}
}
...
function Play(phase)
	while started do
		if not stage[phase].done then
			stage[phase].done = true
			LoadAnim(stage[phase].dict)
			TaskPlayAnim(PlayerPedId(), stage[phase].dict, stage[phase].anim, 8.0, 1.0, -1, 2, 0, 0, 0, 0 )
			Citizen.Wait(stage[phase].wait)
		else
			if phase == #stage then
				reset()
			else
				local coords = GetEntityCoords(PlayerPedId())
				DrawText3Ds(coords.x, coords.y, coords.z, stage[phase].msg)
				if IsControlJustPressed(0, stage[phase].key) then
					break
				end
