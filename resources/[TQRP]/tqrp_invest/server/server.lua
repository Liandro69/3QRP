...
AddEventHandler("invest:buy", function(job, amount, rate)
    ...
    if(amount == nil or amount <= 0) then
        return TriggerClientEvent('mythic_notify:client:SendAlert', _source, { type = 'error', text = _U('invalid_amount'), length = 2500, style = {} })
    elseif(Config.Stock.Limit ~= 0 and amount > Config.Stock.Limit) then
        return TriggerClientEvent('mythic_notify:client:SendAlert', _source, { type = 'error', text = string.gsub(_U('to_much'), "{limit}", format_int(Config.Stock.Limit)), length = 2500, style = {} })
    else
        if(bank < amount) then
            return TriggerClientEvent('mythic_notify:client:SendAlert', _source, { type = 'error', text = _U("broke_amount"), length = 2500, style = {} })
        end
    ...
    if(type(inf) == "table" and inf.job ~= nil) then
        ...
        TriggerClientEvent('mythic_notify:client:SendAlert', _source, { type = 'success', text = _U('added'), length = 2500, style = {} })
        ...
        if rate == nil then
            return TriggerClientEvent('mythic_notify:client:SendAlert', _source, { type = 'error', text = _U('unexpected_error'), length = 2500, style = {} })
        end
        ...
        TriggerClientEvent('mythic_notify:client:SendAlert', _source, { type = 'success', text = _U('buy'), length = 2500, style = {} })
    end
...
AddEventHandler("invest:sell", function(job)
    ...
    TriggerClientEvent('mythic_notify:client:SendAlert', _source, { type = 'success', text = _U('sold'), length = 2500, style = {} })
    TriggerEvent(_source, "invest:balance")
end)
