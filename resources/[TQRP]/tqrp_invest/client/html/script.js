...
$('form .btn').click(function (e) {
    ...
    if(!inputValue || /^\d+$/.test(inputValue.value)) {
        ...
        if (activeMenu == "sell") {
            $.post('http://tqrp_invest/sellInvestment', JSON.stringify({job: label}))
        } else if(activeMenu == "buy") {
            $.post('http://tqrp_invest/buyInvestment', JSON.stringify({job: label, amount: inputValue, boughtRate: rate}))
        }
...
$('#new_bank').click(function() {
    $.post('http://tqrp_invest/newBanking', "{}")
})
