...
cfg.blackjackTables = {
    --[id] = {x,y,z,heading}
    [0] = {
        dealerPos = vector3(1151.24, 267.41, -51.84),
        dealerHeading = 219.34,
        tablePos = vector3(1151.84, 266.747, -52.84095),
        tableHeading = 45.3093,
        distance = 1000.0,
        prop = "vw_prop_casino_blckjack_01"
    },

    [1] = {
        dealerPos = vector3(1149.44, 269.18, -51.84),
        dealerHeading = 45.58,
        tablePos = vector3(1148.837, 269.747, -52.84095),
        tableHeading = 225.309,
        distance = 1000.0,
        prop = "vw_prop_casino_blckjack_01"
    }, 
    [2] = {
        dealerPos = vector3(1128.8, 261.68, -51.04),
        dealerHeading = 314.27,
    ...
    [3] = {
        dealerPos = vector3(1143.78, 246.7, -51.04),
        dealerHeading = 320.0,
...
--[[ cfg.blackjackTables = {
    --[id] = {x,y,z,heading}
    [0] = {
        dealerPos = vector3(989.47,54.02,75.49),
        dealerHeading = -186.0,
        tablePos = vector3(989.4973, 53.18717, 74.48966),
        tableHeading = 3.0,
        distance = 1000.0,
        prop = "vw_prop_casino_blckjack_01"
    },
    ...
    [1] = {
        dealerPos = vector3(987.31,63.77,75.49),
        dealerHeading = 29.9,
        tablePos = vector3(986.8749, 64.57017, 74.48471),
        tableHeading = 208.0,
        distance = 1000.0,
        prop = "vw_prop_casino_blckjack_01"
    },
    ...
    [2] = {
        dealerPos = vector3(994.25,60.23,75.49),
        dealerHeading = 285.19,
        tablePos = vector3(995.0742, 60.43173, 74.49076),
        tableHeading = 103.0,
        distance = 1000.0,
        prop = "vw_prop_casino_blckjack_01"
    },
    ...
    [3] = {
        dealerPos = vector3(984.42,57.74,75.49),
        dealerHeading = 111.0,
        tablePos = vector3(983.6525, 57.50018, 74.49076),
        tableHeading = 288.0,
        distance = 1000.0,
        prop = "vw_prop_casino_blckjack_01"
    },
...
cfg.buyChips = {
	{ ['x'] = 1116.0, ['y'] = 220.02, ['z'] =  -49.34},
	{ ['x'] = 1116.52, ['y'] = 221.48, ['z'] =  -49.34},
	{ ['x'] = 1116.46, ['y'] = 218.11, ['z'] =  -49.34},
}

CreateThread(function()
	while true do
		local coords = GetEntityCoords(GetPlayerPed(-1))    
		local waitChips = 3000

		for k,v in pairs(cfg.buyChips) do
			if GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 35.0 then
				...
				if GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 4.0 then
					DrawMarker(27, v.x, v.y, v.z - 1, 0, 0, 0, 0, 0, 0, 0.5, 0.5, 1.0, 1, 157, 0, 155, false, false, 2, false, false, false, false)
					DrawText3D(v.x, v.y, v.z, "~g~[E]~w~ Comprar/Vender fichas")
					if(IsControlJustPressed(1, 51)) and GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 2.0 then
						OpenBuySellChips()
					end
				end
			end
...
function OpenBuySellChips()
  ESX.UI.Menu.Open(
    'default', GetCurrentResourceName(), 'menu_principal',
    {
        title    = "Fichas (1€ p/ unidade)",
        align    = 'top-left',
        elements = {
        	{label = "Comprar fichas",   value = "buy_chips"},
        	{label = "Vender fichas",   value = "sell_chips"},
        },
    },
    function(data, menu)
        if data.current.value == "buy_chips" then
        	ESX.UI.Menu.CloseAll()

        	ESX.UI.Menu.Open(
            	'dialog', GetCurrentResourceName(), 'buy_chips_menu',
            	{
              		title = "Dinheiro"
            	},
            function(data2, menu2)
            	...
            	if num == nil then
              		...
              		TriggerServerEvent("tqrp_blackjack:server:BuyChips", num)
            	end
            end,
            	function(data2, menu2)
            		ESX.UI.Menu.CloseAll()
        ...
        if data.current.value == "sell_chips" then
        	ESX.UI.Menu.Open(
            	'dialog', GetCurrentResourceName(), 'sell_chips_menu',
            	{
            		title = "Número de Fichas"
            	},
            function(data3, menu3)
            	...
            	if num == nil then
            		...
            		TriggerServerEvent("tqrp_blackjack:server:SellChips", num)
              	end
            end,
	            function(data3, menu3)
	            	ESX.UI.Menu.CloseAll()
...
RegisterCommand("getcasinotable2",function()
    local playerCoords = GetEntityCoords(PlayerPedId())
...
Citizen.CreateThread(function()
    while true do 
        ...
        if drawCurrentHand then 
            ...
            AddTextComponentString("~b~Jogo:~w~ ["..tostring(currentHand).."] ~r~Mesa:~w~ ["..tostring(dealersHand).."] ~g~Aposta:~w~ ["..tostring(currentBetAmount).."]")
...
Citizen.CreateThread(function()
    while true do 
    	local waitingForBetStateWait = 1000
        if waitingForBetState then
        	waitingForBetStateWait = 5
            if IsDisabledControlJustPressed(0, 22) then --Custom Bet [space]
                local tmpInput = getGenericTextInput("Montante para apostar")
            ...
            if IsControlJustPressed(0, 201) then --Submit bet [Enter]
                ...
                if closestChair >= 8 and closestChair <= 15 then
                    if tonumber(currentBetAmount) >= 50000 then
                        ...
                        notify("Aposta mínima: 50000")
                    end
                elseif tonumber(currentBetAmount) >= 0 then
                    TriggerServerEvent("Blackjack:setBlackjackBet",globalGameId,currentBetAmount,closestChair)
                    closestDealerPed = getClosestDealer()
                    PlayAmbientSpeech1(closestDealerPed,"MINIGAME_DEALER_PLACE_CHIPS","SPEECH_PARAMS_FORCE_NORMAL_CLEAR",1) --TODO check this is the right sound?
                    putBetOnTable()
                    ...
                    notify("Montante inválido")
                end
            end
            if IsControlPressed(0, 27) then --Increase bet [pageup]
...
Citizen.CreateThread(function()
    while true do
    	local closestChairWait = 1000
        if closestChair ~= -1 and closestChairDist < 2 then
        	closestChairWait = 5
            if IsControlJustPressed(0, 38) and not sittingAtBlackjackTable then --Sit down [E]
...
Citizen.CreateThread(function()
    while true do
        local waitChairDraw = 1000
        if closeToCasino then
            closestChairDist = 1.5
            closestChair = -1
            local playerCoords = GetEntityCoords(GetPlayerPed(-1))
            for i=0,((#cfg.blackjackTables+1)*4)-1,1 do
                ...
                if distToBlackjackSeat < closestChairDist then 
                    ...
                    if not sittingAtBlackjackTable then
                        DrawText3D(vectorOfBlackjackSeat.x,vectorOfBlackjackSeat.y,vectorOfBlackjackSeat.z+0.5, "~g~[E]~w~ sentar")
                    end
    ...
    while true do
        if blackjackInstructional ~= nil then 
            ...
            if waitingForBetState then 
                ...
                DrawAdvancedNativeText(0.998, 0.935, 0.005, 0.0028, 0.29, "APOSTA:", 255, 255, 255, 255, 0, 0)
        ...
        if drawTimerBar then
            ...
            DrawAdvancedNativeText(1.015, 0.892, 0.005, 0.0028, 0.29, "TEMPO:", 255, 255, 255, 255, 0, 0)
            if timeLeft < 10 then
...
AddEventHandler("Blackjack:beginBetsBlackjack",function(gameID,tableId)
    ...
    Citizen.CreateThread(function()
        ...
        timeLeft = 20
        drawTimerBar = false
        if not bettedThisRound and sittingAtBlackjackTable then
            --print("made blackjackInstructional nil cause you didnt bet")
            drawNativeNotification("Nenhuma aposta realizada")
        end
    end)
end)
...
function goToBlackjackSeat(blackjackSeatID)
    ...
    drawNativeNotification("Espera pela próxima ronda")
    blackjackAnimsToLoad = {
      "anim_casino_b@amb@casino@games@blackjack@dealer",
      "anim_casino_b@amb@casino@games@shared@dealer@",
      "anim_casino_b@amb@casino@games@blackjack@player",  
...
function setupBlackjackMidBetScaleform(scaleform)
    ...
    PushScaleformMovieFunctionParameterInt(1)
    Button(GetControlInstructionalButton(2, 194, true)) -- The button to display
    ButtonMessage("Passar") --BACKSPACE
    PopScaleformMovieFunctionVoid()
...
Citizen.CreateThread(function() while true do Citizen.Wait(10000) collectgarbage() end end) -- Fix RAM leaks by collecting garbage

CreateThread(function()
    while true do
        local waitDrawTPs = 10000
        local coords = GetEntityCoords(GetPlayerPed(-1))
        ----------------------------------------------------------------------------------------------------------- DISCOTECA
        
        if GetDistanceBetweenCoords(coords,  971.77, 51.73, 120.24,  true) < 25.0 then
            waitDrawTPs = 5
            if GetDistanceBetweenCoords(coords,  971.77, 51.73, 120.24,  true) < 2.0 then
                DrawText3D(944.79, 47.83, 80.29, "~g~[E]~w~ descer")
                if (IsControlPressed(1, 51)) then
                    TeleportPed(GetPlayerPed(-1), 965.01, 58.5, 112.55)
                    Wait(2500)
                end
            end
        end

        if GetDistanceBetweenCoords(coords,  965.01, 58.5, 112.55,  true) < 25.0 then
            waitDrawTPs = 5
            if GetDistanceBetweenCoords(coords,  965.01, 58.5, 112.55,  true) < 2.0 then
                DrawText3D(965.01, 58.5, 112.55, "~g~[E]~w~ subir")
                if (IsControlPressed(1, 51)) then
                    TeleportPed(GetPlayerPed(-1), 971.77, 51.73, 120.24)
                    Wait(2500)
                end
            end
        end
...
function TeleportPed(ped,x,y,z)
    DoScreenFadeOut(500)
...
CreateThread(function()
    local blip = AddBlipForCoord(934.32, 42.07, 80.29)
