...
function tryTakeChips(source,amount)
    ...
    if xPlayer.getInventoryItem('chips').count >= amount then --xPlayer.getInventoryItem('chips').count  >= amount then
        --xPlayer.removeInventoryItem('chips', amount)
        xPlayer.removeInventoryItem('chips',amount)
        return true
    else
        TriggerClientEvent('mythic_notify:client:SendAlert', _source , { type = 'error', text = 'Não tens fichas suficientes!' })
        return false
    end
end
...
AddEventHandler("Blackjack:requestSitAtBlackjackTable", function(chairId)
    ...
    if source ~= nil then
        ...
        TriggerClientEvent('mythic_notify:client:SendAlert', source , { type = 'error', text = 'Não te podes sentar aqui!' })
...
AddEventHandler("Blackjack:setBlackjackBet",function(gameId,betAmount,chairId)
    ...
    if gameId ~= nil and betAmount ~= nil and chairId ~= nil then 
            ...
            if tonumber(betAmount) then
                ...
                if betAmount > 0 then
                   ...
                   --     print("Taken",betAmount,"chips from id",source)
                        ...
                        TriggerClientEvent('mythic_notify:client:SendAlert', source , { type = 'success', text = "Aposta feita no valor de ".. tostring(betAmount) .. " fichas!" })
                    else 
                        TriggerClientEvent('mythic_notify:client:SendAlert', source , { type = 'error', text = "Não tens fichas suficientes!" })
        ...
        TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = "Erro de aposta!" })
...
RegisterNetEvent("tqrp_blackjack:server:SellChips")
AddEventHandler("tqrp_blackjack:server:SellChips",function(num)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    if xPlayer.getInventoryItem('chips').count >= num then --xPlayer.getInventoryItem('chips').count  >= amount then
        --xPlayer.removeInventoryItem('chips', amount)
        xPlayer.addMoney(num)
        xPlayer.removeInventoryItem('chips', num)
    else
        TriggerClientEvent('mythic_notify:client:SendAlert', _source , { type = 'error', text = 'Não tens dinheiro!' }) 
    end    
end)



RegisterNetEvent("tqrp_blackjack:server:BuyChips")
AddEventHandler("tqrp_blackjack:server:BuyChips",function(num)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    if xPlayer.getMoney() >= num then --xPlayer.getInventoryItem('chips').count  >= amount then
        --xPlayer.removeInventoryItem('chips', amount)
        xPlayer.removeMoney(num)
        xPlayer.addInventoryItem('chips', num)
    else
        TriggerClientEvent('mythic_notify:client:SendAlert', _source , { type = 'error', text = 'Não tens dinheiro!' }) 
    end    
end)
                                          ...
                                          --  print("response received! [ok]")
                                            if blackjackGameData[gameId][source][2][nextCardCount] == true then --if hit 
                                                ...
                                                if currentHand > 21 then
                                                    ...
                                                    TriggerClientEvent('mythic_notify:client:SendAlert', source , { type = 'error', text = "Perdeste: " .. tostring(lostAmount) .. " fichas!" }) 
                                    ...
                                    if blackjackGameData[gameId][source]["status"] ~= "bust" then 
                                        ...
                                        if currentHand ~= nil then
                                            if currentHand <= 21 then
                                                ...
                                                if currentHand == 21 and dealerHand ~= currentHand then
                                                    potentialWinAmount = blackjackGameData[gameId][source][1] * 3
                                                    ...
                                                    if playerPing ~= nil then
                                                        if playerPing > 0 then
                                                            ...
                                                            TriggerClientEvent("blackjack:notify",source,"Ganhaste "..tostring(potentialWinAmount).." fichas!")
                                                ...
                                                elseif currentHand == dealerHand then
                                                    ...
                                                    if playerPing ~= nil then
                                                        if playerPing > 0 then
                                                            ...
                                                            TriggerClientEvent("blackjack:notify",source,"Empate")
                                                        end
                                                    end
                                                else
                                                    ...
                                                    if playerPing ~= nil then
                                                        if playerPing > 0 then
                                                            ...
                                                            TriggerClientEvent("blackjack:notify",source,"Perdeste "..tostring(potentialPushAmount).." fichas")
                                                            if potentialPushAmount > 10000000 then
