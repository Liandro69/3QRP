GetOutfits = function(source)
  ...
  TriggerEvent('tqrp_datastore:getDataStore', 'housing', xPlayer.identifier, function(store)
...
RegisterServerEvent('tqrp_housing:removeOutfit')
AddEventHandler('tqrp_housing:removeOutfit', function(label)
	local xPlayer = ESX.GetPlayerFromId(source)
	TriggerEvent('tqrp_datastore:getDataStore', 'housing', xPlayer.identifier, function(store)
      ...
      if v.Owner and v.Owner:len() >= 1 then
        ...
        if salePrice > 0 then
          if targetPlayer then
            ...
            NotifyPlayer(targetSource,string.format(Labels["SolddHouse"],v.Price,(v.Price ~= salePrice and Labels["HouseEarning"] or ".")))
          else
...
GetItem = function(house,itemType,itemName,itemCount)
  ...
  if itemType == 'item_money' then
    ...
    TriggerEvent("tqrp_base:itemLog", "CASA", "Retirou: Money x".. itemCount, 9509044, _source)
      ...
      for _,item in pairs(v.Inventory.Items) do
        if item.name == itemName then
          ...
          if xPlayer.canCarryItem(itemName, itemCount) then
            ...
            TriggerEvent("tqrp_base:itemLog", "CASA", "Retirou: ".. itemName .. " x".. itemCount, 9509044, _source)
            --SyncHouse(v)
            SqlExecute("UPDATE "..((Config and Config.AllhousingTable) or "allhousing").." SET inventory=@inventory WHERE id=@id",{['@inventory'] = json.encode(v.Inventory),['@id'] = v.Id})
            return
          else
            NotifyPlayer(_source,"Não consegues carregar tal material.")
            return
          end
        end
      end
...
PutItem = function(house,itemType,itemName,itemCount)
  ...
  if itemType == 'item_money' then
    ...
    v.Inventory.Cash = math.floor(v.Inventory.Cash + itemCount)
    TriggerEvent("tqrp_base:itemLog", "CASA", "Colocou: Money x".. itemCount, 5715253, _source)
    --SyncHouse(v)
    SqlExecute("UPDATE "..((Config and Config.AllhousingTable) or "allhousing").." SET inventory=@inventory WHERE id=@id",{['@inventory'] = json.encode(v.Inventory),['@id'] = v.Id})
  elseif itemType == 'item_account' then
    ...
    if v.Inventory and v.Inventory.Items then
      for _,item in pairs(v.Inventory.Items) do
        if item.name == itemName then
          ...
          TriggerEvent("tqrp_base:itemLog", "CASA", "Colocou: ".. itemName .. " x".. itemCount, 5715253, _source)
          --SyncHouse(v)
          SqlExecute("UPDATE "..((Config and Config.AllhousingTable) or "allhousing").." SET inventory=@inventory WHERE id=@id",{['@inventory'] = json.encode(v.Inventory),['@id'] = v.Id})      
        end
      end
...
AddEventHandler("Allhousing:FullDeleteHouse",function(house)
  ...
  if xPlayer.getGroup() == 'admin' or xPlayer.getGroup() == 'superadmin' then
    ...
    Houses[house.Id] = nil
    TriggerClientEvent('mythic_notify:client:SendAlert', src , { type = 'success', text = 'Casa apagada com sucesso!' })
  else
    TriggerClientEvent('mythic_notify:client:SendAlert', src , { type = 'error', text = 'Não tens permissões para fazer isto!' })
  end
end)
