...
ESXMenuHandler = function(d,t,st)
  ...
  elseif t == "Wardrobe" then
    if st == "Owner" or st == "Owned" or st == "Police" then
      ESXWardrobeMenu(d)
    end
...
ESXWardrobeMenu = function(d)
  ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'dress_menu',
  {
    ...
    elements = 
    {
      {label = "Vestir Roupa", value = 'player_dressing'},
      {label = "Remover Roupa", value = 'remove_cloth'}
    }
  }, function(data, menu)
    if data.current.value == 'player_dressing' then
      ...
      end, function(submitData, submitMenu)
          submitMenu.close()
      end)
    elseif data.current.value == 'remove_cloth' then
      ...
      ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'remove_cloth',
      {
        ...
        menu2.close()
        TriggerServerEvent('tqrp_housing:removeOutfit', data2.current.value)
        exports['mythic_notify']:SendAlert('success', "Apagaste essa roupa!")
