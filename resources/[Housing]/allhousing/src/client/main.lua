...
local stations = {vector3(460.43, -988.31, 30.69), vector3(1859.1212158203,3695.466796875,34.234176635742), vector3(301.47,-600.45,43.28), vector3(1765.15,2591.71,49.71)}
...
Police = function()
  ...
  local job = GetPlayerJobName()
    if job == 'police' or job == 'sheriff' or job == 'ambulance' then
      ...
      local plyPos = GetEntityCoords(plyPed)
      for i = 1, #stations do
        if #(plyPos - stations[i]) < 2.0 then
          res = true
          DrawText3D(stations[i].x, stations[i].y, stations[i].z, "[~r~E~s~]  Mudar de Roupa")
          if IsControlJustPressed(0,38) then
            OpenMenu("Police", "Wardrobe", "Police")
          end
        end
      end
    end
