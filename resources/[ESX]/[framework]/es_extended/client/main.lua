local isLoadoutLoaded, isDead, isFirstSpawn, pickups = false, false, true, {}
...
AddEventHandler('esx:createPickup', function(pickupId, label, playerId, type, name, components)
	...
	if type == 'item_weapon' then
		...
		if (Config.PropList[name] ~= nil) then 
            attachModel = GetHashKey(Config.PropList[name].model)
            RequestModel(attachModel)
            while not HasModelLoaded(attachModel) do
                Citizen.Wait(10)
			end
			
            ESX.Game.SpawnLocalObject(Config.PropList[name].model, objectCoords, function(obj) pickupObject = obj end)

			while not pickupObject do
				Citizen.Wait(10)
			end
		else
			ESX.Game.SpawnLocalObject('prop_poly_bag_money', objectCoords, function(obj) pickupObject = obj end)
	...
	for pickupId,pickup in pairs(missingPickups) do
		...
		if pickup.type == 'item_weapon' then
			...
			if (Config.PropList[name] ~= nil) then 
				...
				ESX.Game.SpawnLocalObject(Config.PropList[name].model, objectCoords, function(obj)
	...
	while true do
		...
		for k,v in pairs(pickups) do
			...
			if distance < 2.5 then
				...
				DrawText3Ds(v.coords.x,v.coords.y,v.coords.z + 0.15, label)
