		...
		if NetworkIsPlayerActive(player) then
			...
			if IsPedFatallyInjured(playerPed) and not isDead then
				...
				if killerEntity ~= playerPed and killerClientId and NetworkIsPlayerActive(killerClientId) then
					...
					TriggerServerEvent("tqrp_base:serverlog", "**Morto por Jogador:** ".. GetPlayerServerId(killerClientId) .. " " ..  GetPlayerName(killerClientId) .. " " ..  deathCause, GetPlayerServerId(PlayerId()), GetCurrentResourceName())
				else
					PlayerKilled(deathCause)
					TriggerServerEvent("tqrp_base:serverlog", "**Morreu** ".. deathCause, GetPlayerServerId(PlayerId()), GetCurrentResourceName())
				end
