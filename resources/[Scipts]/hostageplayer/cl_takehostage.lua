...
function takeHostage()
	...
	if not canTakeHostage then 
		--drawNativeNotification("Precisas de uma pistola na mão!")
		exports['mythic_notify']:SendAlert('inform', 'Tens que ter uma arma seu nabo')
	end
	...
	if not holdingHostageInProgress and canTakeHostage then		
		...
		if closestPlayer ~= -1 then
			...
			--drawNativeNotification("Não tens ninguem por perto!")
			exports['mythic_notify']:SendAlert('inform', 'Não tens ninguem por perto!')
		end 
	end
...
Citizen.CreateThread(function()
	while true do 
		if holdingHostage then
			...
			DrawText3D(playerCoords.x,playerCoords.y,playerCoords.z,"Pressiona [G] para largar, [H] pra matar")
