...
function TokoVoip.updateTokoVoipInfo(self, forceUpdate) -- Update the top-left info
	...
	if (self.plugin_data.radioChannel ~= -1 and self.myChannels[self.plugin_data.radioChannel]) then
		if (string.match(self.myChannels[self.plugin_data.radioChannel].name, "Chamada")) then
			ChannelText = " [Chamada] " .. self.myChannels[self.plugin_data.radioChannel].name;
		else
			ChannelText = " [Rádio] " .. self.myChannels[self.plugin_data.radioChannel].name .. " MHz";
		end
	end
	if self.plugin_data.radioTalking and self.talking == 1 then
		ChannelText = "<font class='talking'>" .. ChannelText .. "</font>";
	end
	if self.talking == 1 then
		info = "<font class='talking'>" .. info .. "</font>";
	end
	if (info .. ChannelText == self.screenInfo and not forceUpdate) then return end
	self.screenInfo = info .. ChannelText;
	self:updatePlugin("updateTokovoipInfo", "" .. info .. ChannelText);
end
	...
	Citizen.CreateThread(function()
		while (true) do
			...
			if ((self.keySwitchChannelsSecondary and IsControlPressed(0, self.keySwitchChannelsSecondary)) or not self.keySwitchChannelsSecondary) then -- Switch radio channels
				if ((IsDisabledControlJustPressed(0, self.keySwitchChannels) or IsControlJustPressed(0, self.keySwitchChannels)) and tablelength(self.myChannels) > 0) then
					local myChannels = {};
			...
			elseif (IsDisabledControlJustPressed(0, self.keyProximity) or IsControlJustPressed(0, self.keyProximity)) then -- Switch proximity modes (normal / whisper / shout)
			...
			if (IsControlPressed(0, self.radioKey) and self.plugin_data.radioChannel ~= -1 and self.config.radioEnabled and --[[not IsPlayerDead(PlayerId())]] not IsEntityDead(PlayerPedId())) then -- Talk on radio
				...
				if (lastTalkState == false and self.myChannels[self.plugin_data.radioChannel] and self.config.radioAnim) then
					if (not string.match(self.myChannels[self.plugin_data.radioChannel].name, "Call") and not IsPedSittingInAnyVehicle(PlayerPedId()) and self.plugin_data.radioChannel < self.config.radioClickMaxChannel) then
