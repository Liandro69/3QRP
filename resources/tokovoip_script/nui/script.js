...
function updateTokovoipInfo(msg) {
	document.getElementById('tokovoipInfo').style.fontSize = '15px';
	document.getElementById('tokovoipInfo').style.fontWeight = '100';
	document.getElementById('tokovoipInfo').style.fontFamily = "Helvetica";
	let screenMessage;
	...
	switch (voipStatus) {
		...
		case PLUGIN_INITIALIZING:
			...
			break;
		case WRONG_SERVER:
			msg = `Conectado ao TeamSpeak Errado ❌`;
			screenMessage = 'Servidor de TS Errado ❌';
			color = 'red';
			break;
		case WRONG_CHANNEL:
			msg = `Conectado à sala errada do TeamSpeak ❌`;
			screenMessage = 'Canal de TS Errado';
			color = 'red';
			break;
		case INCORRECT_VERSION:
			msg = 'Tens o TokoVoip Desatualizado';
			screenMessage = 'Versão merda do Plugin';
			color = 'red';
			break;
		case OK:
			color = "#c7c7c7";
	...
	document.getElementById('pluginStatus').innerHTML = `Estado do Plugin: <font color="${color}">${screenMessage || msg}</font>`;
}
