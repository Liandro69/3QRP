    ...
    while true do
        ...
        if IsPedDeadOrDying(me) then
            for k, v in pairs(receptionPeds) do
                if DoesEntityExist(v) and not Cinema then
                    sleep = 7
                    --if GetDistanceBetweenCoords(GetEntityCoords(me), GetOffsetFromEntityInWorldCoords(v, 0.0, 2.0, 0.0), true) <= 1.5 and not Cinema then 
                        drawTxt((Strings['get_help']):format('[~g~E~w~]', Config.Price))
                        if IsControlJustReleased(0, 38) or IsDisabledControlJustReleased(0, 38) then
                            TriggerServerEvent('esx_beds:gethelp', k)
                            TriggerServerEvent('tqrp_base:serverlog', 'Medico Ilegal. \n Pagou 2000$', GetPlayerServerId(PlayerId()), "tqrp_ilhospital")
                        end
                    --end
                end
            end
        ...
        for k, v in pairs(Config.Hospitals) do
            if #(v.Peds.reception.coords - GetEntityCoords(me)) <= 15.5 then
                if not DoesEntityExist(receptionPeds[k]) then
                    receptionPeds[k] = createPed(v.Peds.pedHash, v.Peds.reception.coords, v.Peds.reception.heading, false)  
                end
            elseif DoesEntityExist(receptionPeds[k]) then
                DeleteEntity(receptionPeds[k])
            end
        end
        Wait(sleep)
    end
...
AddEventHandler('esx_beds:revive', function(hospital)
    ...
    while timer >= GetGameTimer() do drawTxt("Deixa me preparar a maca e falamos...") for i = 0, 31 do DisableAllControlActions(i) end Wait(0) end
