...
local BodyParts = {
    ...
    ['UPPER_BODY'] = { label = 'Parte Superior', causeLimp = false, isDamaged = false, severity = 0 },
    ['LOWER_BODY'] = { label = 'Parte Inferior', causeLimp = true, isDamaged = false, severity = 0 },
    ...
    ['LFINGER'] = { label = 'Dedos Mão Esquerda', causeLimp = false, isDamaged = false, severity = 0 },
    ...
    ['RFINGER'] = { label = 'Dedos Mão Direita', causeLimp = false, isDamaged = false, severity = 0 },
    ['RLEG'] = { label = 'Perda Direita', causeLimp = true, isDamaged = false, severity = 0 },
...
function ProcessRunStuff(ped)
    if IsInjuryCausingLimp() and not (onPainKiller > 0)  then
        ...
        if wasOnPainKillers then
            ...
            exports['mythic_notify']:SendAlert('inform', 'Medicamentos não vão resolver os teus problemas', 5000, { ['background-color'] = '#760036' })
        end
    else
                    ...
                    if not IsPedRagdoll(ped) and IsPedOnFoot(ped) then
                        ...
                        if (IsPedRunning(ped) or IsPedSprinting(ped)) then
                            if chance <= Config.LegInjuryChance.Running then
                                --exports['mythic_notify']:SendAlert('inform', 'Dificuldade em correr', 5000, { ['background-color'] = '#760036' })
