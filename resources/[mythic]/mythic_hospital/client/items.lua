...
AddEventHandler("mythic_hospital:items:firstaid", function(item)
    exports['mythic_progbar']:Progress({
        name = "firstaid_action",
        duration = 10000,
        label = "A aplicar um kit de primeiros socorros...",
        useWhileDead = false,
        canCancel = true,
        controlDisables = {
            disableMovement = false,
...
AddEventHandler("mythic_hospital:items:vicodin", function(item)
    exports['mythic_progbar']:Progress({
        name = "firstaid_action",
        duration = 1000,
        label = "Tomaste comprimidos de " .. item.label,
        useWhileDead = false,
        canCancel = true,
        controlDisables = {
            disableMovement = false,
        ...
        if not status then
            TriggerEvent('mythic_hospital:client:RemoveBleed', 1)
        end
    end)
end)
