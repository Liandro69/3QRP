...
function DoLimbAlert()
    ...
    if not IsEntityDead(player) then
        if #injured > 0 then
            ...
            exports['mythic_notify']:SendAlert('error', limbDamageMsg)
        else
            exports['mythic_notify']:SendAlert('inform', "A dor parou")
        end
    end
end
...
function DoBleedAlert()
    ...
    if not IsEntityDead(player) and isBleeding > 0 then
        exports['mythic_notify']:SendAlert('error', string.format(Config.Strings.BleedAlert, Config.BleedingStates[isBleeding]))
    else
        exports['mythic_notify']:SendAlert('inform', "O Sangramento parou")
    end
end